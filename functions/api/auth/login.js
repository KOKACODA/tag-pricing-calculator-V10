...
async function handle(context) {
  ...
  // —— 首次初始化管理员（USERS 为空且携带正确 SETUP_KEY），初始化后直接返回会话 ——
  const users = await listUsers(env);
  if (!users.length) {
    if (env.SETUP_KEY && body.setupKey === env.SETUP_KEY && env.ADMIN_PASS && env.ADMIN_USER) {
      const username = String(env.ADMIN_USER).trim().toLowerCase();
      const salt = randomSalt();
      const admin = {
        role: 'admin', username,
        nickname: env.ADMIN_NICK || null,
        pwdHash: await hashPassword(env.ADMIN_PASS, salt), salt,
        enabled: true, expiresAt: null,
        createdAt: Date.now(), lastLogin: null
      };
      await saveUser(env, username, admin);
      const sess = await createSession(env, username, admin.role, context.request);
      return json({ ok: true, setup: 'created', username, token: sess.sid, user: publicUser(admin) });
    }

// ============================================================
// KOKALabel报价系统 v10.7.0 - 数据配置层
// ============================================================
"use strict";

// ============================================================
// ===================== 价格配置区（唯一维护区） =====================
// ============================================================
// 【调价仅修改此区域】后续调整价格、纸张、工艺、邮费、客户等级时，
// 只修改下方的配置对象，无需改动其后的计算逻辑与渲染代码。

/**
 * ==================== 数据架构说明 ====================
 *
 * 三级层级关系：
 *   小组（Group） → 总报价表（PriceList） → Sheet 纸张报价表（Paper）
 *
 * - 小组：一个独立团队/账号，拥有自己的总报价表集合。
 *   当前硬编码为单小组，后续接入账号系统后可扩展多小组。
 *   示例：1号小组、2号小组
 *
 * - 总报价表：一个小组的完整报价数据集合，对应一个 Excel 文件。
 *   当前硬编码为"1号报价表"，后续可扩展"2号报价表""3号报价表"等。
 *   导入导出时，一个总报价表 = 一个 Excel 文件（含多个 Sheet）。
 *
 * - Sheet 纸张报价表：总报价表内的单张纸报价数据，对应 Excel 的一个 Sheet。
 *   每张纸含多个规格（code）× 多个档位（tier）的价格矩阵。
 *   当前 9 张纸张，后续不同总报价表可含不同数量的 Sheet（可达 30+）。
 *
 * 当前阶段：单小组 + 单总报价表（1号报价表）+ 9 张 Sheet 纸张
 * 后续扩展：无需改动计算逻辑，仅需增加数据配置或接入 API 拉取。
 * ====================================================
 */

/**
 * 当前小组与总报价表元信息
 */
const GROUP_META = {
  id: "group1",
  name: "1楼小组"
};

// v8.0：小组名映射（按报价表 groupId 取小组名，供导出/展示使用）
const GROUP_NAME_MAP = {
  group1: "1楼小组",
  group2: "3楼小组"
};

// EPHEMERAL_KEYS 提前定义为空数组（不拦截任何 key），供 loadFromStorage 安全调用
// v10.7.0：必须定义在 PRICE_LIST_GROUPS 初始化之前（loadFromStorage 内部引用它，避免 TDZ 报错）
const EPHEMERAL_KEYS = [];

// v10.7.0：报价表组可在线增删改名（默认沿用原有两组，向后兼容旧 localStorage，对齐 v9.8 体系）
const DEFAULT_PRICE_LIST_GROUPS = [
  { id: "group1", name: "1楼小组" },
  { id: "group2", name: "3楼小组" }
];
let PRICE_LIST_GROUPS = loadFromStorage("priceListGroups", DEFAULT_PRICE_LIST_GROUPS.map(g => ({ ...g })));

// 报价表组配置：支持多个报价表，吊绳/邮费按报价表隔离
const DEFAULT_PRICE_LISTS = [
  { id: "priceList1", name: "1楼", groupId: "group1" },
  { id: "priceList2", name: "3楼", groupId: "group2" }
];
let PRICE_LISTS = loadFromStorage("priceLists", DEFAULT_PRICE_LISTS.map(p => ({ ...p })));
let CURRENT_PRICE_LIST_ID = loadFromStorage("currentPriceListId", "priceList1");

/**
 * Sheet 纸张报价表配置（默认配置）
 * specs 必须按面积从小到大排列；007 与 008 必须完全一致；009 与 100 保持原始数据。
 * 每张纸属于当前总报价表（通过 priceListId 字段标识）。
 */
const DEFAULT_PAPER_CONFIG = [
  {
    "id": "paper1",
    "name": "3楼小组-1：350克 A级铜版纸 双面过哑胶（厚度0.38mm）",
    "shortName": "350铜版纸",
    "priceListId": "priceList2",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        500,
        1000,
        2000,
        2500,
        3000,
        5000,
        10000,
        20000,
        50000
      ],
      "max": [
        1.3,
        1.3,
        1.3,
        1.3,
        1.3,
        1.2,
        1.2,
        1.2,
        1.2
      ],
      "min": [
        1.25,
        1.25,
        1.25,
        1.25,
        1.25,
        1.15,
        1.15,
        1.15,
        1.15
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "500": 20,
          "1000": 25,
          "2000": 45,
          "2500": 50,
          "5000": 95,
          "10000": 190,
          "20000": 350,
          "50000": 800
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "500": 25,
          "1000": 30,
          "2000": 55,
          "2500": 60,
          "5000": 105,
          "10000": 200,
          "20000": 400,
          "50000": 1000
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "500": 30,
          "1000": 35,
          "2000": 65,
          "2500": 70,
          "5000": 120,
          "10000": 240,
          "20000": 480,
          "50000": 1150
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "500": 35,
          "1000": 40,
          "2000": 75,
          "2500": 85,
          "5000": 150,
          "10000": 270,
          "20000": 530,
          "50000": 1300
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "500": 40,
          "1000": 45,
          "2000": 85,
          "2500": 90,
          "5000": 160,
          "10000": 300,
          "20000": 580,
          "50000": 1400
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "500": 50,
          "1000": 55,
          "2000": 95,
          "2500": 100,
          "5000": 170,
          "10000": 340,
          "20000": 660,
          "50000": 1600
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "500": 60,
          "1000": 65,
          "2000": 110,
          "2500": 125,
          "5000": 200,
          "10000": 390,
          "20000": 750,
          "50000": 1950
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "500": 60,
          "1000": 65,
          "2000": 110,
          "2500": 125,
          "5000": 200,
          "10000": 390,
          "20000": 750,
          "50000": 1950
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "500": 70,
          "1000": 75,
          "2000": 125,
          "2500": 150,
          "5000": 260,
          "10000": 500,
          "20000": 950,
          "50000": 2300
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "500": 80,
          "1000": 85,
          "2000": 145,
          "2500": 165,
          "5000": 290,
          "10000": 560,
          "20000": 1100,
          "50000": 2550
        }
      }
    ]
  },
  {
    "id": "paper2",
    "name": "3楼小组-2：400克 A级铜版纸 双面过哑胶（厚度0.45mm）",
    "shortName": "400铜版纸",
    "priceListId": "priceList2",
    "discount": 0.91,
    "directCoeff": {
      "tiers": [
        1000,
        2000,
        3000,
        4000,
        5000,
        10000,
        20000,
        50000
      ],
      "max": [
        1.2,
        1.2,
        1.2,
        1.2,
        1.15,
        1.15,
        1.1,
        1.1
      ],
      "min": [
        1.15,
        1.15,
        1.15,
        1.15,
        1.1,
        1.1,
        1.05,
        1.05
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 25,
          "2000": 45,
          "3000": 75,
          "4000": 85,
          "5000": 120,
          "10000": 210,
          "20000": 400,
          "30000": null,
          "50000": 900,
          "100000": null
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 35,
          "2000": 55,
          "3000": 90,
          "4000": 105,
          "5000": 140,
          "10000": 240,
          "20000": 460,
          "30000": null,
          "50000": 1100,
          "100000": null
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 40,
          "2000": 70,
          "3000": 105,
          "4000": 130,
          "5000": 170,
          "10000": 285,
          "20000": 550,
          "30000": null,
          "50000": 1300,
          "100000": null
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 50,
          "2000": 80,
          "3000": 130,
          "4000": 160,
          "5000": 180,
          "10000": 310,
          "20000": 590,
          "30000": null,
          "50000": 1400,
          "100000": null
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 55,
          "2000": 90,
          "3000": 140,
          "4000": 170,
          "5000": 190,
          "10000": 340,
          "20000": 650,
          "30000": null,
          "50000": 1550,
          "100000": null
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 65,
          "2000": 100,
          "3000": 160,
          "4000": 190,
          "5000": 200,
          "10000": 380,
          "20000": 720,
          "30000": null,
          "50000": 1750,
          "100000": null
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 75,
          "2000": 120,
          "3000": 190,
          "4000": 230,
          "5000": 240,
          "10000": 460,
          "20000": 880,
          "30000": null,
          "50000": 2100,
          "100000": null
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 75,
          "2000": 120,
          "3000": 190,
          "4000": 230,
          "5000": 240,
          "10000": 460,
          "20000": 880,
          "30000": null,
          "50000": 2100,
          "100000": null
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 90,
          "2000": 150,
          "3000": 230,
          "4000": 280,
          "5000": 300,
          "10000": 550,
          "20000": 1050,
          "30000": null,
          "50000": 2500,
          "100000": null
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 100,
          "2000": 170,
          "3000": 260,
          "4000": 310,
          "5000": 330,
          "10000": 620,
          "20000": 1180,
          "30000": null,
          "50000": 2700,
          "100000": null
        }
      }
    ]
  },
  {
    "id": "paper3",
    "name": "3楼小组-3：702克 A级铜版纸 双面过哑胶（厚度0.85mm）",
    "shortName": "702铜版纸",
    "priceListId": "priceList2",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        1000,
        2000,
        3000,
        4000,
        5000,
        10000,
        20000,
        50000
      ],
      "max": [
        1.6,
        1.6,
        1.6,
        1.6,
        1.5,
        1.5,
        1.45,
        1.45
      ],
      "min": [
        1.5,
        1.5,
        1.5,
        1.5,
        1.45,
        1.45,
        1.4,
        1.4
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 35,
          "2000": 55,
          "3000": null,
          "4000": null,
          "5000": 115,
          "10000": 220,
          "20000": 420,
          "50000": 1050
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 40,
          "2000": 70,
          "3000": null,
          "4000": null,
          "5000": 135,
          "10000": 260,
          "20000": 500,
          "50000": 1200
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 45,
          "2000": 85,
          "3000": null,
          "4000": null,
          "5000": 165,
          "10000": 310,
          "20000": 600,
          "50000": 1500
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 55,
          "2000": 95,
          "3000": null,
          "4000": null,
          "5000": 180,
          "10000": 340,
          "20000": 660,
          "50000": 1600
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 65,
          "2000": 105,
          "3000": null,
          "4000": null,
          "5000": 200,
          "10000": 380,
          "20000": 730,
          "50000": 1800
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 75,
          "2000": 115,
          "3000": null,
          "4000": null,
          "5000": 220,
          "10000": 410,
          "20000": 800,
          "50000": 1950
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 85,
          "2000": 140,
          "3000": null,
          "4000": null,
          "5000": 260,
          "10000": 480,
          "20000": 960,
          "50000": 2350
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 85,
          "2000": 140,
          "3000": null,
          "4000": null,
          "5000": 260,
          "10000": 480,
          "20000": 960,
          "50000": 2350
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 105,
          "2000": 170,
          "3000": null,
          "4000": null,
          "5000": 310,
          "10000": 590,
          "20000": 1180,
          "50000": 2850
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 120,
          "2000": 185,
          "3000": null,
          "4000": null,
          "5000": 340,
          "10000": 650,
          "20000": 1260,
          "50000": 3000
        }
      }
    ]
  },
  {
    "id": "paper4",
    "name": "3楼小组-4：米白卡 606克（厚度0.8mm）",
    "shortName": "606米白卡",
    "priceListId": "priceList2",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        1000,
        2000,
        2500,
        3000,
        4000,
        5000,
        10000,
        20000,
        50000
      ],
      "max": [
        1.45,
        1.45,
        1.45,
        1.45,
        1.45,
        1.6,
        1.6,
        1.5,
        1.5
      ],
      "min": [
        1.4,
        1.4,
        1.4,
        1.4,
        1.4,
        1.5,
        1.5,
        1.45,
        1.45
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "500": 30,
          "1000": 40,
          "2000": 60,
          "2500": 70,
          "5000": 135,
          "7500": 210,
          "10000": 260,
          "20000": 500,
          "30000": 760,
          "50000": 1250
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "500": 35,
          "1000": 45,
          "2000": 75,
          "2500": 85,
          "5000": 162,
          "7500": 277,
          "10000": 308,
          "20000": 590,
          "30000": 880,
          "50000": 1450
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "500": 45,
          "1000": 55,
          "2000": 95,
          "2500": 105,
          "5000": 200,
          "7500": 305,
          "10000": 380,
          "20000": 730,
          "30000": 1080,
          "50000": 1780
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "500": 55,
          "1000": 65,
          "2000": 105,
          "2500": 115,
          "5000": 220,
          "7500": 335,
          "10000": 420,
          "20000": 800,
          "30000": 1150,
          "50000": 1900
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "500": 65,
          "1000": 75,
          "2000": 115,
          "2500": 125,
          "5000": 240,
          "7500": 365,
          "10000": 460,
          "20000": 880,
          "30000": 1300,
          "50000": 2150
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "500": 75,
          "1000": 85,
          "2000": 125,
          "2500": 135,
          "5000": 260,
          "7500": 395,
          "10000": 500,
          "20000": 950,
          "30000": 1400,
          "50000": 2310
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "500": 95,
          "1000": 105,
          "2000": 155,
          "2500": 165,
          "5000": 320,
          "7500": 485,
          "10000": 610,
          "20000": 1160,
          "30000": 1700,
          "50000": 2800
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "500": 95,
          "1000": 105,
          "2000": 155,
          "2500": 165,
          "5000": 320,
          "7500": 485,
          "10000": 610,
          "20000": 1160,
          "30000": 1700,
          "50000": 2800
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "500": 105,
          "1000": 125,
          "2000": 185,
          "2500": 195,
          "5000": 380,
          "7500": 575,
          "10000": 730,
          "20000": 1400,
          "30000": 2050,
          "50000": 3400
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "500": 125,
          "1000": 145,
          "2000": 205,
          "2500": 215,
          "5000": 410,
          "7500": 625,
          "10000": 780,
          "20000": 1500,
          "30000": 2150,
          "50000": 3550
        }
      }
    ]
  },
  {
    "id": "paper5",
    "name": "3楼小组-5：500克 白卡纸（厚度0.55mm）正面过哑胶",
    "shortName": "500白卡纸",
    "priceListId": "priceList2",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        1000,
        2000,
        3000,
        4000,
        5000,
        10000,
        20000,
        50000
      ],
      "max": [
        1.25,
        1.25,
        1.25,
        1.25,
        1.15,
        1.15,
        1.1,
        1.1
      ],
      "min": [
        1.2,
        1.2,
        1.2,
        1.2,
        1.1,
        1.1,
        1.05,
        1.05
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "500": 25,
          "1000": 35,
          "2000": 65,
          "2500": null,
          "3000": 90,
          "4000": 115,
          "5000": 140,
          "7500": null,
          "10000": 265,
          "20000": 500,
          "30000": 650,
          "50000": 1000
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "500": 30,
          "1000": 40,
          "2000": 75,
          "2500": null,
          "3000": 105,
          "4000": 130,
          "5000": 170,
          "7500": null,
          "10000": 320,
          "20000": 600,
          "30000": 850,
          "50000": 1350
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "500": 35,
          "1000": 45,
          "2000": 80,
          "2500": null,
          "3000": 120,
          "4000": 150,
          "5000": 210,
          "7500": null,
          "10000": 380,
          "20000": 720,
          "30000": 1000,
          "50000": 1650
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "500": 45,
          "1000": 55,
          "2000": 100,
          "2500": null,
          "3000": 145,
          "4000": 180,
          "5000": 220,
          "7500": null,
          "10000": 400,
          "20000": 760,
          "30000": 1100,
          "50000": 1800
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "500": 55,
          "1000": 65,
          "2000": 120,
          "2500": null,
          "3000": 170,
          "4000": 215,
          "5000": 240,
          "7500": null,
          "10000": 430,
          "20000": 820,
          "30000": 1200,
          "50000": 2000
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "500": 65,
          "1000": 75,
          "2000": 135,
          "2500": null,
          "3000": 195,
          "4000": 245,
          "5000": 280,
          "7500": null,
          "10000": 480,
          "20000": 910,
          "30000": 1350,
          "50000": 2200
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "500": 75,
          "1000": 85,
          "2000": 160,
          "2500": null,
          "3000": 230,
          "4000": 280,
          "5000": 320,
          "7500": null,
          "10000": 570,
          "20000": 1090,
          "30000": 1550,
          "50000": 2550
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "500": 75,
          "1000": 85,
          "2000": 160,
          "2500": null,
          "3000": 230,
          "4000": 280,
          "5000": 320,
          "7500": null,
          "10000": 570,
          "20000": 1090,
          "30000": 1550,
          "50000": 2550
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "500": 85,
          "1000": 95,
          "2000": 170,
          "2500": null,
          "3000": 230,
          "4000": 280,
          "5000": 320,
          "7500": null,
          "10000": 570,
          "20000": 1090,
          "30000": 1550,
          "50000": 2550
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "500": 90,
          "1000": 100,
          "2000": 180,
          "2500": null,
          "3000": 270,
          "4000": 330,
          "5000": 380,
          "7500": null,
          "10000": 700,
          "20000": 1330,
          "30000": 1900,
          "50000": 3100
        }
      }
    ]
  },
  {
    "id": "paper6",
    "name": "3楼小组-6：400克米白卡 / 160克半透卡 / 200克合成纸（海成专用 / 撕不烂）",
    "shortName": "海成专用",
    "priceListId": "priceList2",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        1000,
        2000,
        3000,
        4000,
        5000,
        10000,
        20000,
        50000
      ],
      "max": [
        1.2,
        1.2,
        1.2,
        1.2,
        1.15,
        1.15,
        1.1,
        1.1
      ],
      "min": [
        1.15,
        1.15,
        1.15,
        1.15,
        1.1,
        1.1,
        1.05,
        1.05
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 40,
          "2000": 72,
          "3000": 105,
          "4000": 132,
          "5000": 160,
          "10000": 305,
          "20000": 578
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 165,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 58,
          "2000": 105,
          "3000": 150,
          "4000": 190,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 65,
          "2000": 116,
          "3000": 170,
          "4000": 215,
          "5000": 260,
          "10000": 494,
          "20000": 938
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 75,
          "2000": 135,
          "3000": 195,
          "4000": 248,
          "5000": 300,
          "10000": 570,
          "20000": 1083
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 85,
          "2000": 153,
          "3000": 220,
          "4000": 280,
          "5000": 340,
          "10000": 646,
          "20000": 1228
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 110,
          "2000": 198,
          "3000": 285,
          "4000": 363,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 120,
          "2000": 216,
          "3000": 310,
          "4000": 400,
          "5000": 500,
          "10000": 950,
          "20000": 1805
        }
      }
    ]
  },
  {
    "id": "paper7",
    "name": "3楼小组-7：700克 A级布纹纸 双面过光油（厚度0.85mm）",
    "shortName": "700布纹纸",
    "priceListId": "priceList2",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 35,
          "2000": 63,
          "3000": 91,
          "4000": 112,
          "5000": 125,
          "10000": 230,
          "20000": 440
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 42,
          "2000": 77,
          "3000": 120,
          "4000": 150,
          "5000": 160,
          "10000": 305,
          "20000": 580
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 140,
          "4000": 175,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 60,
          "2000": 108,
          "3000": 168,
          "4000": 205,
          "5000": 220,
          "10000": 410,
          "20000": 780
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 66,
          "2000": 115,
          "3000": 175,
          "4000": 220,
          "5000": 240,
          "10000": 450,
          "20000": 850
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 73,
          "2000": 122,
          "3000": 189,
          "4000": 240,
          "5000": 260,
          "10000": 500,
          "20000": 950
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 90,
          "2000": 154,
          "3000": 231,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1150
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 90,
          "2000": 154,
          "3000": 231,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1150
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 112,
          "2000": 175,
          "3000": 266,
          "4000": 330,
          "5000": 400,
          "10000": 760,
          "20000": 1450
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 130,
          "2000": 203,
          "3000": 315,
          "4000": 378,
          "5000": 450,
          "10000": 850,
          "20000": 1650
        }
      }
    ]
  },
  {
    "id": "paper8",
    "name": "3楼小组-8：600克 A级牛皮纸（厚度0.80mm）",
    "shortName": "600牛皮纸",
    "priceListId": "priceList2",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        1000,
        2000,
        3000,
        4000,
        5000,
        10000,
        20000,
        50000
      ],
      "max": [
        1.25,
        1.25,
        1.25,
        1.25,
        1.15,
        1.15,
        1.1,
        1.1
      ],
      "min": [
        1.2,
        1.2,
        1.2,
        1.2,
        1.1,
        1.1,
        1.05,
        1.05
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513,
          "20000": 975
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589,
          "20000": 1120
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703,
          "20000": 1335
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779,
          "20000": 1480
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254,
          "20000": 2383
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406,
          "20000": 2672
        }
      }
    ]
  },
  {
    "id": "paper9",
    "name": "3楼小组-9：棉麻布 40C 米白色",
    "shortName": "40C棉麻布",
    "priceListId": "priceList2",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": {
      "maxArea": 7004,
      "prices": {
        "1000": 168,
        "2000": 330,
        "3000": 500,
        "4000": 650,
        "5000": 780,
        "10000": 1480,
        "20000": 2900
      }
    },
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 90,
          "2000": 160,
          "3000": 235,
          "4000": 300,
          "5000": 360,
          "10000": 680
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 100,
          "2000": 180,
          "3000": 260,
          "4000": 330,
          "5000": 400,
          "10000": 760
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 110,
          "2000": 200,
          "3000": 290,
          "4000": 365,
          "5000": 440,
          "10000": 830
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 120,
          "2000": 220,
          "3000": 315,
          "4000": 400,
          "5000": 480,
          "10000": 880
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 130,
          "2000": 235,
          "3000": 340,
          "4000": 430,
          "5000": 520,
          "10000": 950
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 140,
          "2000": 255,
          "3000": 370,
          "4000": 460,
          "5000": 560,
          "10000": 1000
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 160,
          "2000": 290,
          "3000": 420,
          "4000": 530,
          "5000": 640,
          "10000": 1150
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 160,
          "2000": 290,
          "3000": 420,
          "4000": 530,
          "5000": 640,
          "10000": 1150
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 200,
          "2000": 360,
          "3000": 520,
          "4000": 660,
          "5000": 800,
          "10000": 1400
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 200,
          "2000": 360,
          "3000": 520,
          "4000": 660,
          "5000": 800,
          "10000": 1400
        }
      }
    ]
  },
  {
    "id": "paper10",
    "name": "3楼小组-10：米白卡 602克（厚度0.8mm）",
    "shortName": "602米白卡",
    "priceListId": "priceList2",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        1000,
        2000,
        2500,
        3000,
        4000,
        5000,
        10000,
        20000,
        50000
      ],
      "max": [
        1.45,
        1.45,
        1.45,
        1.45,
        1.45,
        1.6,
        1.6,
        1.5,
        1.5
      ],
      "min": [
        1.4,
        1.4,
        1.4,
        1.4,
        1.4,
        1.5,
        1.5,
        1.45,
        1.45
      ]
    },
    "batchDirect": null,
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 40,
          "2000": 60,
          "2500": null,
          "5000": 135,
          "7500": null,
          "10000": 260,
          "20000": 500,
          "30000": 760,
          "50000": 1250
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 45,
          "2000": 75,
          "2500": null,
          "5000": 162,
          "7500": null,
          "10000": 308,
          "20000": 590,
          "30000": 880,
          "50000": 1450
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 55,
          "2000": 95,
          "2500": null,
          "5000": 200,
          "7500": null,
          "10000": 380,
          "20000": 730,
          "30000": 1080,
          "50000": 1780
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 65,
          "2000": 105,
          "2500": null,
          "5000": 220,
          "7500": null,
          "10000": 420,
          "20000": 800,
          "30000": 1150,
          "50000": 1900
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 75,
          "2000": 115,
          "2500": null,
          "5000": 240,
          "7500": null,
          "10000": 460,
          "20000": 880,
          "30000": 1300,
          "50000": 2150
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 85,
          "2000": 125,
          "2500": null,
          "5000": 260,
          "7500": null,
          "10000": 500,
          "20000": 950,
          "30000": 1400,
          "50000": 2310
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 105,
          "2000": 155,
          "2500": null,
          "5000": 320,
          "7500": null,
          "10000": 610,
          "20000": 1160,
          "30000": 1700,
          "50000": 2800
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 105,
          "2000": 155,
          "2500": null,
          "5000": 320,
          "7500": null,
          "10000": 610,
          "20000": 1160,
          "30000": 1700,
          "50000": 2800
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 125,
          "2000": 185,
          "2500": null,
          "5000": 380,
          "7500": null,
          "10000": 730,
          "20000": 1400,
          "30000": 2050,
          "50000": 3400
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 145,
          "2000": 205,
          "2500": null,
          "5000": 410,
          "7500": null,
          "10000": 780,
          "20000": 1500,
          "30000": 2150,
          "50000": 3550
        }
      }
    ]
  },
  {
    "id": "paper2_1",
    "name": "1楼小组-1：米兰纹 700克 双面过光油",
    "shortName": "700米兰纹",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 35,
          "2000": 63,
          "3000": 91,
          "4000": 112,
          "5000": 125,
          "10000": 230,
          "20000": 440
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 42,
          "2000": 77,
          "3000": 120,
          "4000": 150,
          "5000": 160,
          "10000": 305,
          "20000": 580
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 140,
          "4000": 175,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 60,
          "2000": 108,
          "3000": 168,
          "4000": 205,
          "5000": 220,
          "10000": 410,
          "20000": 780
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 65,
          "2000": 115,
          "3000": 175,
          "4000": 220,
          "5000": 240,
          "10000": 450,
          "20000": 850
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 73,
          "2000": 122,
          "3000": 189,
          "4000": 240,
          "5000": 260,
          "10000": 500,
          "20000": 950
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 90,
          "2000": 154,
          "3000": 231,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1150
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 90,
          "2000": 154,
          "3000": 231,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1150
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 112,
          "2000": 175,
          "3000": 266,
          "4000": 330,
          "5000": 400,
          "10000": 760,
          "20000": 1450
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 130,
          "2000": 203,
          "3000": 315,
          "4000": 378,
          "5000": 450,
          "10000": 850,
          "20000": 1650
        }
      }
    ]
  },
  {
    "id": "paper2_2",
    "name": "1楼小组-2：400克 A级铜版纸 双面过哑胶",
    "shortName": "400铜版纸",
    "discount": 0.91,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000",
        "50000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 25,
          "2000": 45,
          "3000": 75,
          "4000": 85,
          "5000": 120,
          "10000": 210,
          "20000": 400,
          "50000": 900
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 35,
          "2000": 55,
          "3000": 90,
          "4000": 105,
          "5000": 140,
          "10000": 240,
          "20000": 460,
          "50000": 1100
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 40,
          "2000": 70,
          "3000": 105,
          "4000": 130,
          "5000": 170,
          "10000": 285,
          "20000": 550,
          "50000": 1300
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 50,
          "2000": 80,
          "3000": 130,
          "4000": 160,
          "5000": 180,
          "10000": 310,
          "20000": 590,
          "50000": 1400
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 55,
          "2000": 90,
          "3000": 140,
          "4000": 170,
          "5000": 190,
          "10000": 340,
          "20000": 650,
          "50000": 1550
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 65,
          "2000": 100,
          "3000": 160,
          "4000": 190,
          "5000": 200,
          "10000": 380,
          "20000": 720,
          "50000": 1750
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 75,
          "2000": 120,
          "3000": 190,
          "4000": 230,
          "5000": 240,
          "10000": 460,
          "20000": 880,
          "50000": 2100
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 75,
          "2000": 120,
          "3000": 190,
          "4000": 230,
          "5000": 240,
          "10000": 460,
          "20000": 880,
          "50000": 2100
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 90,
          "2000": 150,
          "3000": 230,
          "4000": 280,
          "5000": 300,
          "10000": 550,
          "20000": 1050,
          "50000": 2500
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 100,
          "2000": 170,
          "3000": 260,
          "4000": 310,
          "5000": 330,
          "10000": 620,
          "20000": 1180,
          "50000": 2700
        }
      }
    ]
  },
  {
    "id": "paper2_3",
    "name": "1楼小组-3：702克 A级铜版纸 双面过哑胶（厚度0.85mm）",
    "shortName": "702铜版纸",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000",
        "50000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 35,
          "2000": 55,
          "5000": 115,
          "10000": 220,
          "20000": 420,
          "50000": 1050
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 40,
          "2000": 70,
          "5000": 135,
          "10000": 260,
          "20000": 500,
          "50000": 1200
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 45,
          "2000": 85,
          "5000": 165,
          "10000": 310,
          "20000": 600,
          "50000": 1500
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 55,
          "2000": 95,
          "5000": 180,
          "10000": 340,
          "20000": 660,
          "50000": 1600
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 65,
          "2000": 105,
          "5000": 200,
          "10000": 380,
          "20000": 730,
          "50000": 1800
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 75,
          "2000": 115,
          "5000": 220,
          "10000": 410,
          "20000": 800,
          "50000": 1950
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 85,
          "2000": 140,
          "5000": 260,
          "10000": 480,
          "20000": 960,
          "50000": 2350
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 85,
          "2000": 140,
          "5000": 260,
          "10000": 480,
          "20000": 960,
          "50000": 2350
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 105,
          "2000": 170,
          "5000": 310,
          "10000": 590,
          "20000": 1180,
          "50000": 2850
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 120,
          "2000": 185,
          "5000": 340,
          "10000": 650,
          "20000": 1260,
          "50000": 3000
        }
      }
    ]
  },
  {
    "id": "paper2_4",
    "name": "1楼小组-4：700克 A级布纹纸 双面过光油",
    "shortName": "700布纹纸KOKA报价",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 35,
          "2000": 63,
          "3000": 91,
          "4000": 112,
          "5000": 125,
          "10000": 230,
          "20000": 440
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 42,
          "2000": 77,
          "3000": 120,
          "4000": 150,
          "5000": 160,
          "10000": 305,
          "20000": 580
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 140,
          "4000": 175,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 60,
          "2000": 108,
          "3000": 168,
          "4000": 205,
          "5000": 220,
          "10000": 410,
          "20000": 780
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 66,
          "2000": 115,
          "3000": 175,
          "4000": 220,
          "5000": 240,
          "10000": 450,
          "20000": 850
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 73,
          "2000": 122,
          "3000": 189,
          "4000": 240,
          "5000": 260,
          "10000": 500,
          "20000": 950
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 90,
          "2000": 154,
          "3000": 231,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1150
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 90,
          "2000": 154,
          "3000": 231,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1150
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 112,
          "2000": 175,
          "3000": 266,
          "4000": 330,
          "5000": 400,
          "10000": 760,
          "20000": 1450
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 130,
          "2000": 203,
          "3000": 315,
          "4000": 378,
          "5000": 450,
          "10000": 850,
          "20000": 1650
        }
      }
    ]
  },
  {
    "id": "paper2_5",
    "name": "1楼小组-5：160克半透卡/200克合成纸",
    "shortName": "160半透卡/200克合成纸",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 40,
          "2000": 72,
          "3000": 105,
          "4000": 132,
          "5000": 160,
          "10000": 305,
          "20000": 578
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 165,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 58,
          "2000": 105,
          "3000": 150,
          "4000": 190,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 65,
          "2000": 116,
          "3000": 170,
          "4000": 215,
          "5000": 260,
          "10000": 494,
          "20000": 938
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 75,
          "2000": 135,
          "3000": 195,
          "4000": 248,
          "5000": 300,
          "10000": 570,
          "20000": 1083
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 85,
          "2000": 153,
          "3000": 220,
          "4000": 280,
          "5000": 340,
          "10000": 646,
          "20000": 1228
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 110,
          "2000": 198,
          "3000": 285,
          "4000": 363,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 120,
          "2000": 216,
          "3000": 310,
          "4000": 400,
          "5000": 500,
          "10000": 950,
          "20000": 1805
        }
      }
    ]
  },
  {
    "id": "paper2_6",
    "name": "1楼小组-6：米白卡 606克",
    "shortName": "606米白卡",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "500",
        "1000",
        "2000",
        "2500",
        "5000",
        "7500",
        "10000",
        "20000",
        "30000",
        "50000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "500": 30,
          "1000": 40,
          "2000": 60,
          "2500": 70,
          "5000": 135,
          "7500": 210,
          "10000": 260,
          "20000": 500,
          "30000": 760,
          "50000": 1250
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "500": 35,
          "1000": 45,
          "2000": 75,
          "2500": 85,
          "5000": 162,
          "7500": 277,
          "10000": 308,
          "20000": 590,
          "30000": 880,
          "50000": 1450
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "500": 45,
          "1000": 55,
          "2000": 95,
          "2500": 105,
          "5000": 200,
          "7500": 305,
          "10000": 380,
          "20000": 730,
          "30000": 1080,
          "50000": 1780
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "500": 55,
          "1000": 65,
          "2000": 105,
          "2500": 115,
          "5000": 220,
          "7500": 335,
          "10000": 420,
          "20000": 800,
          "30000": 1150,
          "50000": 1900
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "500": 65,
          "1000": 75,
          "2000": 115,
          "2500": 125,
          "5000": 240,
          "7500": 365,
          "10000": 460,
          "20000": 880,
          "30000": 1300,
          "50000": 2150
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "500": 75,
          "1000": 85,
          "2000": 125,
          "2500": 135,
          "5000": 260,
          "7500": 395,
          "10000": 500,
          "20000": 950,
          "30000": 1400,
          "50000": 2310
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "500": 95,
          "1000": 105,
          "2000": 155,
          "2500": 165,
          "5000": 320,
          "7500": 485,
          "10000": 610,
          "20000": 1160,
          "30000": 1700,
          "50000": 2800
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "500": 95,
          "1000": 105,
          "2000": 155,
          "2500": 165,
          "5000": 320,
          "7500": 485,
          "10000": 610,
          "20000": 1160,
          "30000": 1700,
          "50000": 2800
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "500": 105,
          "1000": 125,
          "2000": 185,
          "2500": 195,
          "5000": 380,
          "7500": 575,
          "10000": 730,
          "20000": 1400,
          "30000": 2050,
          "50000": 3400
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "500": 125,
          "1000": 145,
          "2000": 205,
          "2500": 215,
          "5000": 410,
          "7500": 625,
          "10000": 780,
          "20000": 1500,
          "30000": 2150,
          "50000": 3550
        }
      }
    ]
  },
  {
    "id": "paper2_7",
    "name": "1楼小组-7：500克 白卡纸 正面过哑胶",
    "shortName": "500白卡纸",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        "500",
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000",
        "30000",
        "50000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "500": 25,
          "1000": 35,
          "2000": 65,
          "3000": 90,
          "4000": 115,
          "5000": 140,
          "10000": 265,
          "20000": 500,
          "30000": 650,
          "50000": 1000
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "500": 30,
          "1000": 40,
          "2000": 75,
          "3000": 105,
          "4000": 130,
          "5000": 170,
          "10000": 320,
          "20000": 600,
          "30000": 850,
          "50000": 1350
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "500": 35,
          "1000": 45,
          "2000": 80,
          "3000": 120,
          "4000": 150,
          "5000": 210,
          "10000": 380,
          "20000": 720,
          "30000": 1000,
          "50000": 1650
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "500": 45,
          "1000": 55,
          "2000": 100,
          "3000": 145,
          "4000": 180,
          "5000": 220,
          "10000": 400,
          "20000": 760,
          "30000": 1100,
          "50000": 1800
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "500": 55,
          "1000": 65,
          "2000": 120,
          "3000": 170,
          "4000": 215,
          "5000": 240,
          "10000": 430,
          "20000": 820,
          "30000": 1200,
          "50000": 2000
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "500": 65,
          "1000": 75,
          "2000": 135,
          "3000": 195,
          "4000": 245,
          "5000": 280,
          "10000": 480,
          "20000": 910,
          "30000": 1350,
          "50000": 2200
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "500": 75,
          "1000": 85,
          "2000": 160,
          "3000": 230,
          "4000": 280,
          "5000": 320,
          "10000": 570,
          "20000": 1090,
          "30000": 1550,
          "50000": 2550
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "500": 75,
          "1000": 85,
          "2000": 160,
          "3000": 230,
          "4000": 280,
          "5000": 320,
          "10000": 570,
          "20000": 1090,
          "30000": 1550,
          "50000": 2550
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "500": 90,
          "1000": 100,
          "2000": 180,
          "3000": 270,
          "4000": 330,
          "5000": 380,
          "10000": 700,
          "20000": 1330,
          "30000": 1900,
          "50000": 3100
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "500": 100,
          "1000": 110,
          "2000": 200,
          "3000": 300,
          "4000": 365,
          "5000": 430,
          "10000": 800,
          "20000": 1500,
          "30000": 2100,
          "50000": 3300
        }
      }
    ]
  },
  {
    "id": "paper2_8",
    "name": "1楼小组-8：素棉纸 700克",
    "shortName": "700素棉纸",
    "discount": 0.85,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513,
          "20000": 975
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589,
          "20000": 1120
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703,
          "20000": 1335
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779,
          "20000": 1480
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254,
          "20000": 2383
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406,
          "20000": 2672
        }
      }
    ]
  },
  {
    "id": "paper2_9",
    "name": "1楼小组-9：800克 艺术棉卡 含四色彩印",
    "shortName": "800艺术棉卡",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406
        }
      }
    ]
  },
  {
    "id": "paper2_10",
    "name": "1楼小组-10：白纹卡600/黑卡纸600/牛皮纸600",
    "shortName": "600白纹/600黑卡/600牛皮",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513,
          "20000": 975
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589,
          "20000": 1120
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703,
          "20000": 1335
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779,
          "20000": 1480
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254,
          "20000": 2383
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406,
          "20000": 2672
        }
      }
    ]
  },
  {
    "id": "paper2_33",
    "name": "1楼小组-33：纹棉纸800",
    "shortName": "800纹棉",
    "discount": 1.2,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513,
          "20000": 975
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589,
          "20000": 1120
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703,
          "20000": 1335
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779,
          "20000": 1480
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254,
          "20000": 2383
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406,
          "20000": 2672
        }
      }
    ]
  },
  {
    "id": "paper2_11",
    "name": "1楼小组-11：米白卡400/牛皮纸350/大地纸200/木香纸400/牛油纸160/磨砂片25C",
    "shortName": "400米白/350牛皮/200大地/400木香/160牛油/25c磨砂片",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 40,
          "2000": 72,
          "3000": 105,
          "4000": 132,
          "5000": 160,
          "10000": 305,
          "20000": 578
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 165,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 58,
          "2000": 105,
          "3000": 150,
          "4000": 190,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 65,
          "2000": 116,
          "3000": 170,
          "4000": 215,
          "5000": 260,
          "10000": 494,
          "20000": 938
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 75,
          "2000": 135,
          "3000": 195,
          "4000": 248,
          "5000": 300,
          "10000": 570,
          "20000": 1083
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 85,
          "2000": 153,
          "3000": 220,
          "4000": 280,
          "5000": 340,
          "10000": 646,
          "20000": 1228
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 110,
          "2000": 198,
          "3000": 285,
          "4000": 363,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 120,
          "2000": 216,
          "3000": 310,
          "4000": 400,
          "5000": 500,
          "10000": 950,
          "20000": 1805
        }
      }
    ]
  },
  {
    "id": "paper2_29",
    "name": "1楼小组-29：木香纸400/牛油纸160/磨砂片25C",
    "shortName": "400木香/160牛油/25c磨砂片",
    "discount": 1.2,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 40,
          "2000": 72,
          "3000": 105,
          "4000": 132,
          "5000": 160,
          "10000": 305,
          "20000": 578
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 165,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 58,
          "2000": 105,
          "3000": 150,
          "4000": 190,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 65,
          "2000": 116,
          "3000": 170,
          "4000": 215,
          "5000": 260,
          "10000": 494,
          "20000": 938
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 75,
          "2000": 135,
          "3000": 195,
          "4000": 248,
          "5000": 300,
          "10000": 570,
          "20000": 1083
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 85,
          "2000": 153,
          "3000": 220,
          "4000": 280,
          "5000": 340,
          "10000": 646,
          "20000": 1228
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 110,
          "2000": 198,
          "3000": 285,
          "4000": 363,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 120,
          "2000": 216,
          "3000": 310,
          "4000": 400,
          "5000": 500,
          "10000": 950,
          "20000": 1805
        }
      }
    ]
  },
  {
    "id": "paper2_12",
    "name": "1楼小组-12：棉麻布 40C 米白色",
    "shortName": "40C棉麻布",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 90,
          "2000": 160,
          "3000": 235,
          "4000": 300,
          "5000": 360,
          "10000": 680
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 100,
          "2000": 180,
          "3000": 260,
          "4000": 330,
          "5000": 400,
          "10000": 760
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 110,
          "2000": 200,
          "3000": 290,
          "4000": 365,
          "5000": 440,
          "10000": 830
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 120,
          "2000": 220,
          "3000": 315,
          "4000": 400,
          "5000": 480,
          "10000": 880
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 130,
          "2000": 235,
          "3000": 340,
          "4000": 430,
          "5000": 520,
          "10000": 950
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 140,
          "2000": 255,
          "3000": 370,
          "4000": 460,
          "5000": 560,
          "10000": 1000
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 160,
          "2000": 290,
          "3000": 420,
          "4000": 530,
          "5000": 640,
          "10000": 1150
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 160,
          "2000": 290,
          "3000": 420,
          "4000": 530,
          "5000": 640,
          "10000": 1150
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 200,
          "2000": 360,
          "3000": 520,
          "4000": 660,
          "5000": 800,
          "10000": 1400
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 260,
          "2000": 470,
          "3000": 680,
          "4000": 860,
          "5000": 1050,
          "10000": 1900
        }
      }
    ]
  },
  {
    "id": "paper2_13",
    "name": "1楼小组-13：木香纸 800克 含4色彩印",
    "shortName": "800木香纸",
    "discount": 0.75,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 90,
          "2000": 160,
          "3000": 235,
          "4000": 300,
          "5000": 350,
          "10000": 665,
          "20000": 1260
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 780,
          "20000": 1480
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 125,
          "2000": 216,
          "3000": 310,
          "4000": 395,
          "5000": 470,
          "10000": 895,
          "20000": 1700
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 480,
          "5000": 560,
          "10000": 1060,
          "20000": 2020
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 160,
          "2000": 288,
          "3000": 415,
          "4000": 528,
          "5000": 620,
          "10000": 1180,
          "20000": 2240
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 175,
          "2000": 315,
          "3000": 455,
          "4000": 580,
          "5000": 680,
          "10000": 1230,
          "20000": 2340
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 210,
          "2000": 378,
          "3000": 545,
          "4000": 690,
          "5000": 820,
          "10000": 1560,
          "20000": 2960
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 210,
          "2000": 378,
          "3000": 545,
          "4000": 690,
          "5000": 820,
          "10000": 1560,
          "20000": 2960
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 250,
          "2000": 450,
          "3000": 650,
          "4000": 825,
          "5000": 980,
          "10000": 1860,
          "20000": 3530
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 300,
          "2000": 540,
          "3000": 780,
          "4000": 990,
          "5000": 1150,
          "10000": 2180,
          "20000": 4140
        }
      }
    ]
  },
  {
    "id": "paper2_14",
    "name": "1楼小组-14：雅莲纹 900克 含4色彩印 双面过光油",
    "shortName": "900雅莲纹",
    "discount": 0.7,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 72,
          "2000": 132,
          "3000": 186,
          "4000": 234,
          "5000": 276,
          "10000": 525
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 84,
          "2000": 151,
          "3000": 219,
          "4000": 276,
          "5000": 324,
          "10000": 616
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 96,
          "2000": 174,
          "3000": 250,
          "4000": 318,
          "5000": 372,
          "10000": 707
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 114,
          "2000": 204,
          "3000": 294,
          "4000": 376,
          "5000": 444,
          "10000": 844
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 126,
          "2000": 228,
          "3000": 328,
          "4000": 414,
          "5000": 492,
          "10000": 935
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 138,
          "2000": 246,
          "3000": 360,
          "4000": 456,
          "5000": 540,
          "10000": 1026
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 174,
          "2000": 312,
          "3000": 450,
          "4000": 570,
          "5000": 672,
          "10000": 1277
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 174,
          "2000": 312,
          "3000": 450,
          "4000": 570,
          "5000": 672,
          "10000": 1277
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 204,
          "2000": 367,
          "3000": 528,
          "4000": 660,
          "5000": 792,
          "10000": 1505
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 228,
          "2000": 408,
          "3000": 593,
          "4000": 744,
          "5000": 888,
          "10000": 1687
        }
      }
    ]
  },
  {
    "id": "paper2_15",
    "name": "1楼小组-15：手柔皮200/米兰卡200",
    "shortName": "200手柔皮/200米兰",
    "discount": 0.85,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 78,
          "2000": 140,
          "3000": 205,
          "4000": 260,
          "5000": 310,
          "10000": 593,
          "20000": 1125
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 90,
          "2000": 162,
          "3000": 235,
          "4000": 298,
          "5000": 360,
          "10000": 685,
          "20000": 1300
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 102,
          "2000": 183,
          "3000": 265,
          "4000": 336,
          "5000": 410,
          "10000": 775,
          "20000": 1470
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 295,
          "4000": 375,
          "5000": 456,
          "10000": 865,
          "20000": 1640
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 295,
          "4000": 375,
          "5000": 456,
          "10000": 865,
          "20000": 1640
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 130,
          "2000": 238,
          "3000": 243,
          "4000": 435,
          "5000": 540,
          "10000": 1026,
          "20000": 1950
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 372,
          "4000": 480,
          "5000": 600,
          "10000": 1140,
          "20000": 2166
        }
      },
      {
        "code": "120",
        "maxArea": 12000,
        "prices": {
          "1000": 170,
          "2000": 302,
          "3000": 438,
          "4000": 550,
          "5000": 670,
          "10000": 1200
        }
      },
      {
        "code": "140",
        "maxArea": 14000,
        "prices": {
          "1000": 190,
          "2000": 345,
          "3000": 500,
          "4000": 636,
          "5000": 768,
          "10000": 1380
        }
      },
      {
        "code": "160",
        "maxArea": 16000,
        "prices": {
          "1000": 215,
          "2000": 385,
          "3000": 560,
          "4000": 720,
          "5000": 865,
          "10000": 1500
        }
      },
      {
        "code": "180",
        "maxArea": 18000,
        "prices": {
          "1000": 240,
          "2000": 430,
          "3000": 625,
          "4000": 792,
          "5000": 860,
          "10000": 1620
        }
      },
      {
        "code": "200",
        "maxArea": 20000,
        "prices": {
          "1000": 265,
          "2000": 475,
          "3000": 686,
          "4000": 876,
          "5000": 1056,
          "10000": 1800
        }
      }
    ]
  },
  {
    "id": "paper2_32",
    "name": "1楼小组-32：闪银卡180",
    "shortName": "180闪银",
    "discount": 1.5,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 78,
          "2000": 140,
          "3000": 205,
          "4000": 260,
          "5000": 310,
          "10000": 593,
          "20000": 1125
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 90,
          "2000": 162,
          "3000": 235,
          "4000": 298,
          "5000": 360,
          "10000": 685,
          "20000": 1300
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 102,
          "2000": 183,
          "3000": 265,
          "4000": 336,
          "5000": 410,
          "10000": 775,
          "20000": 1470
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 295,
          "4000": 375,
          "5000": 456,
          "10000": 865,
          "20000": 1640
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 295,
          "4000": 375,
          "5000": 456,
          "10000": 865,
          "20000": 1640
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 130,
          "2000": 238,
          "3000": 243,
          "4000": 435,
          "5000": 540,
          "10000": 1026,
          "20000": 1950
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 372,
          "4000": 480,
          "5000": 600,
          "10000": 1140,
          "20000": 2166
        }
      },
      {
        "code": "120",
        "maxArea": 12000,
        "prices": {
          "1000": 170,
          "2000": 302,
          "3000": 438,
          "4000": 550,
          "5000": 670,
          "10000": 1200
        }
      },
      {
        "code": "140",
        "maxArea": 14000,
        "prices": {
          "1000": 190,
          "2000": 345,
          "3000": 500,
          "4000": 636,
          "5000": 768,
          "10000": 1380
        }
      },
      {
        "code": "160",
        "maxArea": 16000,
        "prices": {
          "1000": 215,
          "2000": 385,
          "3000": 560,
          "4000": 720,
          "5000": 865,
          "10000": 1500
        }
      },
      {
        "code": "180",
        "maxArea": 18000,
        "prices": {
          "1000": 240,
          "2000": 430,
          "3000": 625,
          "4000": 792,
          "5000": 860,
          "10000": 1620
        }
      },
      {
        "code": "200",
        "maxArea": 20000,
        "prices": {
          "1000": 265,
          "2000": 475,
          "3000": 686,
          "4000": 876,
          "5000": 1056,
          "10000": 1800
        }
      }
    ]
  },
  {
    "id": "paper2_31",
    "name": "1楼小组-31：云彩纸200",
    "shortName": "200云彩",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 78,
          "2000": 140,
          "3000": 205,
          "4000": 260,
          "5000": 310,
          "10000": 593,
          "20000": 1125
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 90,
          "2000": 162,
          "3000": 235,
          "4000": 298,
          "5000": 360,
          "10000": 685,
          "20000": 1300
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 102,
          "2000": 183,
          "3000": 265,
          "4000": 336,
          "5000": 410,
          "10000": 775,
          "20000": 1470
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 295,
          "4000": 375,
          "5000": 456,
          "10000": 865,
          "20000": 1640
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 295,
          "4000": 375,
          "5000": 456,
          "10000": 865,
          "20000": 1640
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 130,
          "2000": 238,
          "3000": 243,
          "4000": 435,
          "5000": 540,
          "10000": 1026,
          "20000": 1950
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 372,
          "4000": 480,
          "5000": 600,
          "10000": 1140,
          "20000": 2166
        }
      },
      {
        "code": "120",
        "maxArea": 12000,
        "prices": {
          "1000": 170,
          "2000": 302,
          "3000": 438,
          "4000": 550,
          "5000": 670,
          "10000": 1200
        }
      },
      {
        "code": "140",
        "maxArea": 14000,
        "prices": {
          "1000": 190,
          "2000": 345,
          "3000": 500,
          "4000": 636,
          "5000": 768,
          "10000": 1380
        }
      },
      {
        "code": "160",
        "maxArea": 16000,
        "prices": {
          "1000": 215,
          "2000": 385,
          "3000": 560,
          "4000": 720,
          "5000": 865,
          "10000": 1500
        }
      },
      {
        "code": "180",
        "maxArea": 18000,
        "prices": {
          "1000": 240,
          "2000": 430,
          "3000": 625,
          "4000": 792,
          "5000": 860,
          "10000": 1620
        }
      },
      {
        "code": "200",
        "maxArea": 20000,
        "prices": {
          "1000": 265,
          "2000": 475,
          "3000": 686,
          "4000": 876,
          "5000": 1056,
          "10000": 1800
        }
      }
    ]
  },
  {
    "id": "paper2_16",
    "name": "1楼小组-16：亮黑卡800/墨绿卡700",
    "shortName": "800亮黑卡/700墨绿卡",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513,
          "20000": 975
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589,
          "20000": 1120
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703,
          "20000": 1335
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779,
          "20000": 1480
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254,
          "20000": 2383
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406,
          "20000": 2672
        }
      }
    ]
  },
  {
    "id": "paper2_17",
    "name": "1楼小组-17：350克 A级铜版纸 双面过哑胶（厚度0.38mm）",
    "shortName": "350铜版纸",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "500",
        "1000",
        "2000",
        "2500",
        "5000",
        "10000",
        "20000",
        "50000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "500": 20,
          "1000": 25,
          "2000": 45,
          "2500": 50,
          "5000": 95,
          "10000": 190,
          "20000": 350,
          "50000": 800
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "500": 25,
          "1000": 30,
          "2000": 55,
          "2500": 60,
          "5000": 105,
          "10000": 200,
          "20000": 400,
          "50000": 1000
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "500": 30,
          "1000": 35,
          "2000": 65,
          "2500": 70,
          "5000": 120,
          "10000": 240,
          "20000": 480,
          "50000": 1150
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "500": 35,
          "1000": 40,
          "2000": 75,
          "2500": 85,
          "5000": 150,
          "10000": 270,
          "20000": 530,
          "50000": 1300
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "500": 40,
          "1000": 45,
          "2000": 85,
          "2500": 90,
          "5000": 160,
          "10000": 300,
          "20000": 580,
          "50000": 1400
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "500": 50,
          "1000": 55,
          "2000": 95,
          "2500": 100,
          "5000": 170,
          "10000": 340,
          "20000": 680,
          "50000": 1600
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "500": 60,
          "1000": 65,
          "2000": 110,
          "2500": 125,
          "5000": 200,
          "10000": 390,
          "20000": 750,
          "50000": 1950
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "500": 60,
          "1000": 65,
          "2000": 110,
          "2500": 125,
          "5000": 200,
          "10000": 390,
          "20000": 750,
          "50000": 1950
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "500": 70,
          "1000": 75,
          "2000": 125,
          "2500": 150,
          "5000": 260,
          "10000": 500,
          "20000": 950,
          "50000": 2300
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "500": 80,
          "1000": 85,
          "2000": 145,
          "2500": 165,
          "5000": 290,
          "10000": 560,
          "20000": 1100,
          "50000": 2550
        }
      }
    ]
  },
  {
    "id": "paper2_18",
    "name": "1楼小组-18：700克 A级铜版纸 双面过哑胶（厚度0.88mm）",
    "shortName": "700铜版纸",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000",
        "50000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 45,
          "2000": 80,
          "3000": 110,
          "4000": 140,
          "5000": 160,
          "10000": 290,
          "20000": 520,
          "50000": 1100
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 135,
          "4000": 165,
          "5000": 190,
          "10000": 340,
          "20000": 610,
          "50000": 1400
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 160,
          "4000": 200,
          "5000": 230,
          "10000": 410,
          "20000": 740,
          "50000": 1700
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 70,
          "2000": 125,
          "3000": 180,
          "4000": 230,
          "5000": 260,
          "10000": 470,
          "20000": 850,
          "50000": 2000
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 80,
          "2000": 140,
          "3000": 200,
          "4000": 260,
          "5000": 280,
          "10000": 500,
          "20000": 900,
          "50000": 2150
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 90,
          "2000": 150,
          "3000": 220,
          "4000": 280,
          "5000": 300,
          "10000": 540,
          "20000": 970,
          "50000": 2300
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 105,
          "2000": 180,
          "3000": 270,
          "4000": 340,
          "5000": 380,
          "10000": 660,
          "20000": 1200,
          "50000": 2600
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 105,
          "2000": 180,
          "3000": 270,
          "4000": 340,
          "5000": 380,
          "10000": 660,
          "20000": 1200,
          "50000": 2600
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 125,
          "2000": 210,
          "3000": 320,
          "4000": 390,
          "5000": 480,
          "10000": 860,
          "20000": 1550,
          "50000": 3400
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 145,
          "2000": 240,
          "3000": 360,
          "4000": 440,
          "5000": 530,
          "10000": 950,
          "20000": 1700,
          "50000": 3800
        }
      }
    ]
  },
  {
    "id": "paper2_19",
    "name": "1楼小组-19：白纹卡600/黑卡纸600/牛皮纸600/米白卡600",
    "shortName": "600白纹/600黑卡/600牛皮/600米白",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513,
          "20000": 975
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589,
          "20000": 1120
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703,
          "20000": 1335
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779,
          "20000": 1480
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254,
          "20000": 2383
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406,
          "20000": 2672
        }
      }
    ]
  },
  {
    "id": "paper2_34",
    "name": "1楼小组-34：木香纸800",
    "shortName": "800木香",
    "discount": 1.5,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513,
          "20000": 975
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589,
          "20000": 1120
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703,
          "20000": 1335
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779,
          "20000": 1480
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254,
          "20000": 2383
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406,
          "20000": 2672
        }
      }
    ]
  },
  {
    "id": "paper2_20",
    "name": "1楼小组-20：700克 A级布纹纸 双面过光油（厚度0.85mm）",
    "shortName": "700布纹纸海洋厂报价",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 160,
          "5000": 180,
          "10000": 330,
          "20000": 620
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 170,
          "4000": 210,
          "5000": 230,
          "10000": 430,
          "20000": 840
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 70,
          "2000": 130,
          "3000": 200,
          "4000": 250,
          "5000": 280,
          "10000": 545,
          "20000": 1000
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 85,
          "2000": 155,
          "3000": 240,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1180
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 95,
          "2000": 165,
          "3000": 250,
          "4000": 310,
          "5000": 340,
          "10000": 650,
          "20000": 1280
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 105,
          "2000": 175,
          "3000": 270,
          "4000": 340,
          "5000": 380,
          "10000": 720,
          "20000": 1400
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 130,
          "2000": 220,
          "3000": 330,
          "4000": 410,
          "5000": 480,
          "10000": 930,
          "20000": 1850
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 130,
          "2000": 220,
          "3000": 330,
          "4000": 410,
          "5000": 480,
          "10000": 930,
          "20000": 1850
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 160,
          "2000": 250,
          "3000": 380,
          "4000": 470,
          "5000": 580,
          "10000": 1050,
          "20000": 2050
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 185,
          "2000": 290,
          "3000": 450,
          "4000": 540,
          "5000": 650,
          "10000": 1200,
          "20000": 2350
        }
      }
    ]
  },
  {
    "id": "paper2_21",
    "name": "1楼小组-21：200克 合成纸 撕不烂",
    "shortName": "200合成纸",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 40,
          "2000": 72,
          "3000": 105,
          "4000": 132,
          "5000": 160,
          "10000": 305,
          "20000": 578
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 165,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 58,
          "2000": 105,
          "3000": 150,
          "4000": 190,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 65,
          "2000": 116,
          "3000": 170,
          "4000": 215,
          "5000": 260,
          "10000": 494,
          "20000": 938
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 75,
          "2000": 135,
          "3000": 195,
          "4000": 248,
          "5000": 300,
          "10000": 570,
          "20000": 1083
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 85,
          "2000": 153,
          "3000": 220,
          "4000": 280,
          "5000": 340,
          "10000": 646,
          "20000": 1228
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 110,
          "2000": 198,
          "3000": 285,
          "4000": 363,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 120,
          "2000": 216,
          "3000": 310,
          "4000": 400,
          "5000": 500,
          "10000": 950,
          "20000": 1805
        }
      }
    ]
  },
  {
    "id": "paper2_22",
    "name": "1楼小组-22：900克 A级铜版纸 双面过哑胶（厚度1.05mm）",
    "shortName": "900铜版纸",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000",
        "30000",
        "50000",
        "100000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 160,
          "5000": 180,
          "10000": 320,
          "20000": 620,
          "30000": 900,
          "50000": 1380,
          "100000": 2500
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 170,
          "4000": 210,
          "5000": 230,
          "10000": 430,
          "20000": 840,
          "30000": 1200,
          "50000": 1900,
          "100000": 3500
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 70,
          "2000": 130,
          "3000": 200,
          "4000": 250,
          "5000": 280,
          "10000": 545,
          "20000": 1000,
          "30000": 1500,
          "50000": 2350,
          "100000": 4200
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 90,
          "2000": 155,
          "3000": 240,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1180,
          "30000": 1750,
          "50000": 2760,
          "100000": 5050
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 100,
          "2000": 165,
          "3000": 250,
          "4000": 310,
          "5000": 340,
          "10000": 650,
          "20000": 1280,
          "30000": 1900,
          "50000": 3000,
          "100000": 5650
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 110,
          "2000": 175,
          "3000": 270,
          "4000": 340,
          "5000": 380,
          "10000": 720,
          "20000": 1400,
          "30000": 2100,
          "50000": 3200,
          "100000": 6000
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 130,
          "2000": 220,
          "3000": 330,
          "4000": 410,
          "5000": 480,
          "10000": 930,
          "20000": 1850,
          "30000": 2550,
          "50000": 3850,
          "100000": 7200
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 130,
          "2000": 220,
          "3000": 330,
          "4000": 410,
          "5000": 480,
          "10000": 930,
          "20000": 1850,
          "30000": 2550,
          "50000": 3850,
          "100000": 7200
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 160,
          "2000": 250,
          "3000": 380,
          "4000": 470,
          "5000": 580,
          "10000": 1050,
          "20000": 2050,
          "30000": 2950,
          "50000": 4560,
          "100000": 8650
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 290,
          "3000": 450,
          "4000": 540,
          "5000": 650,
          "10000": 1200,
          "20000": 2350,
          "30000": 3300,
          "50000": 5100,
          "100000": 9600
        }
      }
    ]
  },
  {
    "id": "paper2_23",
    "name": "1楼小组-23：600克 A级牛皮纸 双面过哑胶（厚度0.80mm）",
    "shortName": "600牛皮纸",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 155,
          "4000": 195,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 70,
          "2000": 126,
          "3000": 182,
          "4000": 230,
          "5000": 270,
          "10000": 513,
          "20000": 975
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 80,
          "2000": 145,
          "3000": 208,
          "4000": 265,
          "5000": 310,
          "10000": 589,
          "20000": 1120
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 245,
          "4000": 313,
          "5000": 370,
          "10000": 703,
          "20000": 1335
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 105,
          "2000": 190,
          "3000": 273,
          "4000": 345,
          "5000": 410,
          "10000": 779,
          "20000": 1480
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 300,
          "4000": 380,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 375,
          "4000": 475,
          "5000": 560,
          "10000": 1064,
          "20000": 2022
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 170,
          "2000": 306,
          "3000": 440,
          "4000": 550,
          "5000": 660,
          "10000": 1254,
          "20000": 2383
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 190,
          "2000": 340,
          "3000": 495,
          "4000": 620,
          "5000": 740,
          "10000": 1406,
          "20000": 2672
        }
      }
    ]
  },
  {
    "id": "paper2_24",
    "name": "1楼小组-24：85C A级白卡纸 双面过光胶（厚度0.95mm）",
    "shortName": "85C白卡纸",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 160,
          "5000": 180,
          "10000": 320,
          "20000": 620
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 60,
          "2000": 110,
          "3000": 170,
          "4000": 210,
          "5000": 230,
          "10000": 430,
          "20000": 840
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 70,
          "2000": 130,
          "3000": 200,
          "4000": 250,
          "5000": 280,
          "10000": 545,
          "20000": 1000
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 85,
          "2000": 155,
          "3000": 240,
          "4000": 290,
          "5000": 320,
          "10000": 600,
          "20000": 1180
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 95,
          "2000": 165,
          "3000": 250,
          "4000": 310,
          "5000": 340,
          "10000": 650,
          "20000": 1280
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 105,
          "2000": 175,
          "3000": 270,
          "4000": 340,
          "5000": 380,
          "10000": 720,
          "20000": 1400
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 130,
          "2000": 220,
          "3000": 330,
          "4000": 410,
          "5000": 480,
          "10000": 930,
          "20000": 1850
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 130,
          "2000": 220,
          "3000": 330,
          "4000": 410,
          "5000": 480,
          "10000": 930,
          "20000": 1850
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 160,
          "2000": 250,
          "3000": 380,
          "4000": 470,
          "5000": 580,
          "10000": 1050,
          "20000": 2050
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 185,
          "2000": 290,
          "3000": 450,
          "4000": 540,
          "5000": 650,
          "10000": 1200,
          "20000": 2350
        }
      }
    ]
  },
  {
    "id": "paper2_25",
    "name": "1楼小组-25：双铜纸 200克（厚度0.22mm）",
    "shortName": "200双铜纸",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 35,
          "2000": 65,
          "3000": 90,
          "4000": 115,
          "5000": 140,
          "10000": 280,
          "20000": 560
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 40,
          "2000": 80,
          "3000": 105,
          "4000": 135,
          "5000": 170,
          "10000": 320,
          "20000": 630
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 165,
          "5000": 200,
          "10000": 350,
          "20000": 680
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 60,
          "2000": 100,
          "3000": 155,
          "4000": 190,
          "5000": 220,
          "10000": 400,
          "20000": 780
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 70,
          "2000": 125,
          "3000": 180,
          "4000": 230,
          "5000": 250,
          "10000": 500,
          "20000": 980
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 70,
          "2000": 125,
          "3000": 180,
          "4000": 230,
          "5000": 250,
          "10000": 500,
          "20000": 980
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 80,
          "2000": 150,
          "3000": 210,
          "4000": 260,
          "5000": 300,
          "10000": 600,
          "20000": 1150
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 90,
          "2000": 165,
          "3000": 235,
          "4000": 300,
          "5000": 330,
          "10000": 660,
          "20000": 1300
        }
      },
      {
        "code": "140",
        "maxArea": 14000,
        "prices": {
          "1000": 140,
          "2000": 252,
          "3000": 350,
          "4000": 430,
          "5000": 500,
          "10000": 850
        }
      },
      {
        "code": "180",
        "maxArea": 18000,
        "prices": {
          "1000": 160,
          "2000": 280,
          "3000": 400,
          "4000": 480,
          "5000": 550,
          "10000": 950
        }
      },
      {
        "code": "220",
        "maxArea": 22000,
        "prices": {
          "1000": 180,
          "2000": 320,
          "3000": 450,
          "4000": 550,
          "5000": 650,
          "10000": 1050
        }
      },
      {
        "code": "260",
        "maxArea": 26000,
        "prices": {
          "1000": 200,
          "2000": 350,
          "3000": 500,
          "4000": 600,
          "5000": 750,
          "10000": 1150
        }
      },
      {
        "code": "300",
        "maxArea": 30000,
        "prices": {
          "1000": 220,
          "2000": 380,
          "3000": 550,
          "4000": 700,
          "5000": 850,
          "10000": 1300
        }
      },
      {
        "code": "400",
        "maxArea": 40000,
        "prices": {
          "1000": 320,
          "2000": 520,
          "3000": 720,
          "4000": 860,
          "5000": 1000,
          "10000": 1450
        }
      },
      {
        "code": "500",
        "maxArea": 50000,
        "prices": {
          "1000": 360,
          "2000": 600,
          "3000": 800,
          "4000": 1000,
          "5000": 1100,
          "10000": 1650
        }
      },
      {
        "code": "680",
        "maxArea": 68000,
        "prices": {
          "1000": 420,
          "2000": 750,
          "3000": 950,
          "4000": 1150,
          "5000": 1250,
          "10000": 1950
        }
      }
    ]
  },
  {
    "id": "paper2_26",
    "name": "1楼小组-26：厚胶带 200克（厚度0.20mm）含正面4色彩印 背面黑油墨",
    "shortName": "200厚胶带",
    "discount": 0.8,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 78,
          "2000": 140,
          "3000": 205,
          "4000": 260,
          "5000": 310,
          "10000": 593,
          "20000": 1125
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 90,
          "2000": 162,
          "3000": 235,
          "4000": 298,
          "5000": 360,
          "10000": 685,
          "20000": 1300
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 102,
          "2000": 183,
          "3000": 265,
          "4000": 336,
          "5000": 410,
          "10000": 775,
          "20000": 1470
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 295,
          "4000": 375,
          "5000": 456,
          "10000": 865,
          "20000": 1640
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 115,
          "2000": 205,
          "3000": 295,
          "4000": 375,
          "5000": 456,
          "10000": 865,
          "20000": 1640
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 130,
          "2000": 238,
          "3000": 343,
          "4000": 435,
          "5000": 540,
          "10000": 1026,
          "20000": 1950
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 145,
          "2000": 260,
          "3000": 372,
          "4000": 480,
          "5000": 600,
          "10000": 1140,
          "20000": 2166
        }
      },
      {
        "code": "120",
        "maxArea": 12000,
        "prices": {
          "1000": 170,
          "2000": 302,
          "3000": 438,
          "4000": 550,
          "5000": 670,
          "10000": 1200
        }
      },
      {
        "code": "140",
        "maxArea": 14000,
        "prices": {
          "1000": 190,
          "2000": 345,
          "3000": 500,
          "4000": 636,
          "5000": 768,
          "10000": 1380
        }
      },
      {
        "code": "160",
        "maxArea": 16000,
        "prices": {
          "1000": 215,
          "2000": 385,
          "3000": 560,
          "4000": 720,
          "5000": 865,
          "10000": 1500
        }
      },
      {
        "code": "180",
        "maxArea": 18000,
        "prices": {
          "1000": 240,
          "2000": 430,
          "3000": 625,
          "4000": 792,
          "5000": 860,
          "10000": 1620
        }
      },
      {
        "code": "200",
        "maxArea": 20000,
        "prices": {
          "1000": 265,
          "2000": 475,
          "3000": 686,
          "4000": 876,
          "5000": 1056,
          "10000": 1800
        }
      }
    ]
  },
  {
    "id": "paper2_27",
    "name": "1楼小组-27：巴黎白300/160克彩透卡",
    "shortName": "巴黎白彩透卡",
    "discount": 0.8,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000",
        "20000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 40,
          "2000": 72,
          "3000": 105,
          "4000": 132,
          "5000": 160,
          "10000": 305,
          "20000": 578
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 50,
          "2000": 90,
          "3000": 130,
          "4000": 165,
          "5000": 200,
          "10000": 380,
          "20000": 720
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 58,
          "2000": 105,
          "3000": 150,
          "4000": 190,
          "5000": 230,
          "10000": 437,
          "20000": 830
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 65,
          "2000": 116,
          "3000": 170,
          "4000": 215,
          "5000": 260,
          "10000": 494,
          "20000": 938
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 75,
          "2000": 135,
          "3000": 195,
          "4000": 248,
          "5000": 300,
          "10000": 570,
          "20000": 1083
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 85,
          "2000": 153,
          "3000": 220,
          "4000": 280,
          "5000": 340,
          "10000": 646,
          "20000": 1228
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 95,
          "2000": 170,
          "3000": 246,
          "4000": 313,
          "5000": 380,
          "10000": 720,
          "20000": 1368
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 110,
          "2000": 198,
          "3000": 285,
          "4000": 363,
          "5000": 450,
          "10000": 855,
          "20000": 1625
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 120,
          "2000": 216,
          "3000": 310,
          "4000": 400,
          "5000": 500,
          "10000": 950,
          "20000": 1805
        }
      },
      {
        "code": "120",
        "maxArea": 12000,
        "prices": {
          "1000": 140,
          "2000": 252,
          "3000": 365,
          "4000": 460,
          "5000": 560,
          "10000": 1000
        }
      },
      {
        "code": "140",
        "maxArea": 14000,
        "prices": {
          "1000": 160,
          "2000": 288,
          "3000": 416,
          "4000": 530,
          "5000": 640,
          "10000": 1150
        }
      },
      {
        "code": "160",
        "maxArea": 16000,
        "prices": {
          "1000": 180,
          "2000": 320,
          "3000": 468,
          "4000": 600,
          "5000": 720,
          "10000": 1250
        }
      },
      {
        "code": "180",
        "maxArea": 18000,
        "prices": {
          "1000": 200,
          "2000": 360,
          "3000": 520,
          "4000": 660,
          "5000": 800,
          "10000": 1350
        }
      },
      {
        "code": "200",
        "maxArea": 20000,
        "prices": {
          "1000": 220,
          "2000": 395,
          "3000": 572,
          "4000": 730,
          "5000": 880,
          "10000": 1500
        }
      }
    ]
  },
  {
    "id": "paper2_28",
    "name": "1楼小组-28：900克 白金卡（独家款）",
    "shortName": "900白金卡",
    "discount": 1,
    "directCoeff": {
      "tiers": [
        "1000",
        "2000",
        "3000",
        "4000",
        "5000",
        "10000"
      ],
      "max": [
        null,
        null,
        null,
        null,
        null,
        null
      ],
      "min": [
        null,
        null,
        null,
        null,
        null,
        null
      ]
    },
    "batchDirect": null,
    "priceListId": "priceList1",
    "specs": [
      {
        "code": "002",
        "maxArea": 1999,
        "prices": {
          "1000": 80,
          "2000": 156,
          "3000": 232,
          "4000": 305,
          "5000": 380,
          "10000": 700
        }
      },
      {
        "code": "003",
        "maxArea": 3164,
        "prices": {
          "1000": 95,
          "2000": 185,
          "3000": 275,
          "4000": 360,
          "5000": 450,
          "10000": 800
        }
      },
      {
        "code": "004",
        "maxArea": 3999,
        "prices": {
          "1000": 106,
          "2000": 206,
          "3000": 308,
          "4000": 403,
          "5000": 500,
          "10000": 900
        }
      },
      {
        "code": "005",
        "maxArea": 5500,
        "prices": {
          "1000": 125,
          "2000": 245,
          "3000": 363,
          "4000": 475,
          "5000": 590,
          "10000": 1000
        }
      },
      {
        "code": "055",
        "maxArea": 6000,
        "prices": {
          "1000": 140,
          "2000": 275,
          "3000": 406,
          "4000": 532,
          "5000": 660,
          "10000": 1100
        }
      },
      {
        "code": "006",
        "maxArea": 6264,
        "prices": {
          "1000": 155,
          "2000": 303,
          "3000": 450,
          "4000": 590,
          "5000": 730,
          "10000": 1200
        }
      },
      {
        "code": "007",
        "maxArea": 7999,
        "prices": {
          "1000": 195,
          "2000": 380,
          "3000": 565,
          "4000": 740,
          "5000": 920,
          "10000": 1400
        }
      },
      {
        "code": "008",
        "maxArea": 7999,
        "prices": {
          "1000": 195,
          "2000": 380,
          "3000": 565,
          "4000": 740,
          "5000": 920,
          "10000": 1400
        }
      },
      {
        "code": "009",
        "maxArea": 9064,
        "prices": {
          "1000": 225,
          "2000": 440,
          "3000": 653,
          "4000": 855,
          "5000": 1060,
          "10000": 1600
        }
      },
      {
        "code": "100",
        "maxArea": 10000,
        "prices": {
          "1000": 260,
          "2000": 510,
          "3000": 755,
          "4000": 990,
          "5000": 1220,
          "10000": 1800
        }
      }
    ]
  }
];

const DEFAULT_CRAFT_CONFIG = {
  "paper2": [
    {
      "id": "craft2_1",
      "name": "烫金（单面）",
      "prices": {
        "1000": 90,
        "2000": 110,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 200,
        "20000": 280,
        "30000": 350,
        "50000": 450,
        "100000": 800
      }
    },
    {
      "id": "craft2_2",
      "name": "烫金（双面）",
      "prices": {
        "1000": 110,
        "2000": 130,
        "3000": 150,
        "4000": 160,
        "5000": 200,
        "10000": 240,
        "20000": 380,
        "30000": 500,
        "50000": 800,
        "100000": 1400
      }
    },
    {
      "id": "craft2_3",
      "name": "UV",
      "prices": {
        "1000": 100,
        "2000": 100,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 180,
        "20000": 240,
        "30000": 300,
        "50000": 400,
        "100000": 600
      }
    },
    {
      "id": "craft2_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": 600,
        "30000": 800,
        "50000": 1300,
        "100000": 2500
      }
    },
    {
      "id": "craft2_5",
      "name": "凹凸",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": 280,
        "30000": 320,
        "50000": 400,
        "100000": 500
      }
    }
  ],
  "paper3": [
    {
      "id": "craft3_1",
      "name": "无色压凹（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null,
        "50000": null
      }
    },
    {
      "id": "craft3_2",
      "name": "套字压凹（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null,
        "50000": null
      }
    },
    {
      "id": "craft3_3",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null,
        "50000": null
      }
    },
    {
      "id": "craft3_4",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null,
        "50000": null
      }
    },
    {
      "id": "craft3_5",
      "name": "击凸（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null,
        "50000": null
      }
    }
  ],
  "paper4": [
    {
      "id": "craft4_1",
      "name": "无色压凹（单面）",
      "prices": {
        "500": null,
        "1000": 30,
        "2000": 40,
        "2500": null,
        "5000": 70,
        "7500": null,
        "10000": 130,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft4_2",
      "name": "套字压凹（单面）",
      "prices": {
        "500": null,
        "1000": 80,
        "2000": 90,
        "2500": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft4_3",
      "name": "烫金 小面积（单面）",
      "prices": {
        "500": null,
        "1000": 50,
        "2000": 60,
        "2500": null,
        "5000": 90,
        "7500": null,
        "10000": 170,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft4_4",
      "name": "烫金 小面积（双面）",
      "prices": {
        "500": null,
        "1000": 70,
        "2000": 80,
        "2500": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft4_5",
      "name": "击凸（单面）",
      "prices": {
        "500": null,
        "1000": 80,
        "2000": 90,
        "2500": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    }
  ],
  "paper5": [
    {
      "id": "craft5_1",
      "name": "无色压凹（单面）",
      "prices": {
        "500": null,
        "1000": 30,
        "2000": 40,
        "2500": null,
        "3000": null,
        "4000": null,
        "5000": 70,
        "7500": null,
        "10000": 130,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft5_2",
      "name": "套字压凹（单面）",
      "prices": {
        "500": null,
        "1000": 80,
        "2000": 90,
        "2500": null,
        "3000": null,
        "4000": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft5_3",
      "name": "烫金 小面积（单面）",
      "prices": {
        "500": null,
        "1000": 50,
        "2000": 60,
        "2500": null,
        "3000": null,
        "4000": null,
        "5000": 90,
        "7500": null,
        "10000": 170,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft5_4",
      "name": "烫金 小面积（双面）",
      "prices": {
        "500": null,
        "1000": 70,
        "2000": 80,
        "2500": null,
        "3000": null,
        "4000": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft5_5",
      "name": "击凸（单面）",
      "prices": {
        "500": null,
        "1000": 80,
        "2000": 90,
        "2500": null,
        "3000": null,
        "4000": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    }
  ],
  "paper6": [
    {
      "id": "craft6_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft6_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft6_3",
      "name": "鸡眼（单面）",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft6_4",
      "name": "鸡眼 （对折两张）",
      "prices": {
        "1000": 60,
        "2000": 120,
        "3000": 180,
        "4000": 240,
        "5000": 300,
        "10000": 550,
        "20000": null
      }
    }
  ],
  "paper7": [
    {
      "id": "craft7_1",
      "name": "烫金小面积(单面）",
      "prices": {
        "1000": 90,
        "2000": 110,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft7_2",
      "name": "烫金小面积(双面）",
      "prices": {
        "1000": 110,
        "2000": 130,
        "3000": 150,
        "4000": 160,
        "5000": 200,
        "10000": 240,
        "20000": null
      }
    },
    {
      "id": "craft7_3",
      "name": "凹凸",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft7_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft7_5",
      "name": "无色压凹（单面）",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft7_6",
      "name": "深凹烫金（单面）",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    }
  ],
  "paper8": [
    {
      "id": "craft8_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft8_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft8_3",
      "name": "凹凸",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft8_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft8_5",
      "name": "丝印白（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft8_6",
      "name": "丝印白（双面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 90,
        "5000": 90,
        "10000": 160,
        "20000": null
      }
    }
  ],
  "paper9": [
    {
      "id": "craft9_1",
      "name": "专色（单面）",
      "prices": {
        "1000": 90,
        "2000": 120,
        "3000": 150,
        "4000": 180,
        "5000": 200,
        "10000": 300
      }
    },
    {
      "id": "craft9_2",
      "name": "黑白图案（单面）",
      "prices": {
        "1000": 30,
        "2000": 50,
        "3000": 70,
        "4000": 90,
        "5000": 120,
        "10000": 200
      }
    },
    {
      "id": "craft9_3",
      "name": "红色（单面）",
      "prices": {
        "1000": 30,
        "2000": 50,
        "3000": 70,
        "4000": 90,
        "5000": 120,
        "10000": 200
      }
    }
  ],
  "paper10": [
    {
      "id": "craft10_1",
      "name": "压凹+烫金（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "2500": null,
        "5000": 70,
        "7500": null,
        "10000": 130,
        "20000": null,
        "30000": null,
        "50000": null
      }
    }
  ],
  "paper2_1": [
    {
      "id": "craft1_1",
      "name": "无色压凹（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft1_2",
      "name": "套字压凹（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft1_3",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft1_4",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft1_5",
      "name": "击凸（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    }
  ],
  "paper2_2": [
    {
      "id": "craft2_1",
      "name": "烫金（单面）",
      "prices": {
        "1000": 90,
        "2000": 110,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 200,
        "20000": 280,
        "50000": 450
      }
    },
    {
      "id": "craft2_2",
      "name": "烫金（双面）",
      "prices": {
        "1000": 110,
        "2000": 130,
        "3000": 150,
        "4000": 160,
        "5000": 200,
        "10000": 240,
        "20000": 380,
        "50000": 800
      }
    },
    {
      "id": "craft2_3",
      "name": "UV",
      "prices": {
        "1000": 100,
        "2000": 130,
        "3000": 140,
        "4000": 160,
        "5000": 180,
        "10000": 240,
        "20000": 300,
        "50000": 600
      }
    },
    {
      "id": "craft2_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": 600,
        "50000": 1300
      }
    },
    {
      "id": "craft2_5",
      "name": "凹凸",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": 280,
        "50000": 400
      }
    }
  ],
  "paper2_3": [
    {
      "id": "craft3_1",
      "name": "无色压凹（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null,
        "50000": null
      }
    },
    {
      "id": "craft3_2",
      "name": "套字压凹（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null,
        "50000": null
      }
    },
    {
      "id": "craft3_3",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null,
        "50000": null
      }
    },
    {
      "id": "craft3_4",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null,
        "50000": null
      }
    },
    {
      "id": "craft3_5",
      "name": "击凸（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null,
        "50000": null
      }
    }
  ],
  "paper2_4": [
    {
      "id": "craft4_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 90,
        "2000": 110,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft4_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 110,
        "2000": 130,
        "3000": 150,
        "4000": 160,
        "5000": 200,
        "10000": 240,
        "20000": null
      }
    },
    {
      "id": "craft4_3",
      "name": "凹凸",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft4_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft4_5",
      "name": "无色压凹（单面）",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft4_6",
      "name": "深凹烫金（单面）",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    }
  ],
  "paper2_5": [
    {
      "id": "craft5_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft5_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft5_3",
      "name": "鸡眼（单面）",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft5_4",
      "name": "鸡眼（对折两张）",
      "prices": {
        "1000": 60,
        "2000": 120,
        "3000": 180,
        "4000": 240,
        "5000": 300,
        "10000": 550,
        "20000": null
      }
    }
  ],
  "paper2_6": [
    {
      "id": "craft6_1",
      "name": "无色压凹（单面）",
      "prices": {
        "500": null,
        "1000": 30,
        "2000": 40,
        "2500": null,
        "5000": 70,
        "7500": null,
        "10000": 130,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft6_2",
      "name": "套字压凹（单面）",
      "prices": {
        "500": null,
        "1000": 80,
        "2000": 90,
        "2500": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft6_3",
      "name": "烫金 小面积（单面）",
      "prices": {
        "500": null,
        "1000": 50,
        "2000": 60,
        "2500": null,
        "5000": 90,
        "7500": null,
        "10000": 170,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft6_4",
      "name": "烫金 小面积（双面）",
      "prices": {
        "500": null,
        "1000": 70,
        "2000": 80,
        "2500": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    },
    {
      "id": "craft6_5",
      "name": "击凸（单面）",
      "prices": {
        "500": null,
        "1000": 80,
        "2000": 90,
        "2500": null,
        "5000": 120,
        "7500": null,
        "10000": 230,
        "20000": null,
        "30000": null,
        "50000": null
      }
    }
  ],
  "paper2_9": [
    {
      "id": "craft9_1",
      "name": "压凹（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130
      }
    },
    {
      "id": "craft9_2",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170
      }
    },
    {
      "id": "craft9_3",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230
      }
    },
    {
      "id": "craft9_4",
      "name": "凹凸（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230
      }
    }
  ],
  "paper2_10": [
    {
      "id": "craft10_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft10_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft10_3",
      "name": "凹凸",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft10_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft10_5",
      "name": "深凹烫金 无色压凹（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft10_6",
      "name": "丝印白（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft10_7",
      "name": "丝印白（双面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 160,
        "20000": null
      }
    }
  ],
  "paper2_33": [
    {
      "id": "craft33_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft33_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft33_3",
      "name": "凹凸",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft33_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft33_5",
      "name": "深凹烫金 无色压凹（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft33_6",
      "name": "丝印白（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft33_7",
      "name": "丝印白（双面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 160,
        "20000": null
      }
    }
  ],
  "paper2_11": [
    {
      "id": "craft11_1",
      "name": "烫金（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft11_2",
      "name": "烫金（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft11_3",
      "name": "PVC 4色彩印（磨砂片）",
      "prices": {
        "1000": 450,
        "2000": 480,
        "3000": 500,
        "4000": 520,
        "5000": 550,
        "10000": 650,
        "20000": null
      }
    },
    {
      "id": "craft11_4",
      "name": "过白或黑UV油（磨砂片）",
      "prices": {
        "1000": 90,
        "2000": 100,
        "3000": 110,
        "4000": 120,
        "5000": 150,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft11_5",
      "name": "鸡眼（对折两张）",
      "prices": {
        "1000": 60,
        "2000": 120,
        "3000": 180,
        "4000": 240,
        "5000": 300,
        "10000": 550,
        "20000": null
      }
    }
  ],
  "paper2_29": [
    {
      "id": "craft29_1",
      "name": "烫金（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft29_2",
      "name": "烫金（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft29_3",
      "name": "PVC 4色彩印（磨砂片）",
      "prices": {
        "1000": 450,
        "2000": 480,
        "3000": 500,
        "4000": 520,
        "5000": 550,
        "10000": 650,
        "20000": null
      }
    },
    {
      "id": "craft29_4",
      "name": "过白或黑UV油（磨砂片）",
      "prices": {
        "1000": 90,
        "2000": 100,
        "3000": 110,
        "4000": 120,
        "5000": 150,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft29_5",
      "name": "鸡眼（对折两张）",
      "prices": {
        "1000": 60,
        "2000": 120,
        "3000": 180,
        "4000": 240,
        "5000": 300,
        "10000": 550,
        "20000": null
      }
    }
  ],
  "paper2_12": [
    {
      "id": "craft12_1",
      "name": "专色（单面）",
      "prices": {
        "1000": 90,
        "2000": 120,
        "3000": 150,
        "4000": 180,
        "5000": 200,
        "10000": 300
      }
    },
    {
      "id": "craft12_2",
      "name": "黑白图案（单面）",
      "prices": {
        "1000": 30,
        "2000": 50,
        "3000": 70,
        "4000": 90,
        "5000": 120,
        "10000": 200
      }
    },
    {
      "id": "craft12_3",
      "name": "红色（单面）",
      "prices": {
        "1000": 30,
        "2000": 50,
        "3000": 70,
        "4000": 90,
        "5000": 120,
        "10000": 200
      }
    }
  ],
  "paper2_13": [
    {
      "id": "craft13_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft13_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft13_3",
      "name": "无色压凹（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft13_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    }
  ],
  "paper2_14": [
    {
      "id": "craft14_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170
      }
    },
    {
      "id": "craft14_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230
      }
    }
  ],
  "paper2_15": [
    {
      "id": "craft15_1",
      "name": "压凹（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    }
  ],
  "paper2_32": [
    {
      "id": "craft32_1",
      "name": "压凹（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    }
  ],
  "paper2_31": [
    {
      "id": "craft31_1",
      "name": "压凹（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    }
  ],
  "paper2_16": [
    {
      "id": "craft16_1",
      "name": "双面白UV（背面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft16_2",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 190,
        "20000": null
      }
    },
    {
      "id": "craft16_3",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft16_4",
      "name": "古凸（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft16_5",
      "name": "墨绿卡 立体烫金 木★立体凸（正面）",
      "prices": {
        "1000": 120,
        "2000": 150,
        "3000": 180,
        "4000": 210,
        "5000": 240,
        "10000": 450,
        "20000": null
      }
    },
    {
      "id": "craft16_6",
      "name": "透明UV（单面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft16_7",
      "name": "透明UV（双面）",
      "prices": {
        "1000": 80,
        "2000": 100,
        "3000": 110,
        "4000": 120,
        "5000": 130,
        "10000": 220,
        "20000": null
      }
    }
  ],
  "paper2_18": [
    {
      "id": "craft18_1",
      "name": "烫金（单面）",
      "prices": {
        "1000": 90,
        "2000": 110,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 200,
        "20000": 280,
        "50000": 450
      }
    },
    {
      "id": "craft18_2",
      "name": "烫金（双面）",
      "prices": {
        "1000": 110,
        "2000": 130,
        "3000": 150,
        "4000": 160,
        "5000": 200,
        "10000": 240,
        "20000": 380,
        "50000": 800
      }
    },
    {
      "id": "craft18_3",
      "name": "UV",
      "prices": {
        "1000": 100,
        "2000": 130,
        "3000": 140,
        "4000": 160,
        "5000": 180,
        "10000": 240,
        "20000": 300,
        "50000": 600
      }
    },
    {
      "id": "craft18_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": 600,
        "50000": 1300
      }
    },
    {
      "id": "craft18_5",
      "name": "凹凸",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": 280,
        "50000": 400
      }
    }
  ],
  "paper2_19": [
    {
      "id": "craft19_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft19_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft19_3",
      "name": "凹凸",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft19_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft19_5",
      "name": "深凹烫金 无色压凹（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft19_6",
      "name": "无色压凹 米白卡特价（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft19_7",
      "name": "丝印白（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft19_8",
      "name": "丝印白（双面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 160,
        "20000": null
      }
    },
    {
      "id": "craft19_9",
      "name": "黑卡纸UV（单面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft19_10",
      "name": "黑卡纸UV（双面）",
      "prices": {
        "1000": 80,
        "2000": 100,
        "3000": 110,
        "4000": 120,
        "5000": 130,
        "10000": 220,
        "20000": null
      }
    }
  ],
  "paper2_34": [
    {
      "id": "craft34_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft34_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft34_3",
      "name": "凹凸",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft34_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft34_5",
      "name": "深凹烫金 无色压凹（单面）",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft34_6",
      "name": "无色压凹 米白卡特价（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft34_7",
      "name": "丝印白（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft34_8",
      "name": "丝印白（双面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 160,
        "20000": null
      }
    },
    {
      "id": "craft34_9",
      "name": "黑卡纸UV（单面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft34_10",
      "name": "黑卡纸UV（双面）",
      "prices": {
        "1000": 80,
        "2000": 100,
        "3000": 110,
        "4000": 120,
        "5000": 130,
        "10000": 220,
        "20000": null
      }
    }
  ],
  "paper2_20": [
    {
      "id": "craft20_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 90,
        "2000": 110,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft20_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 110,
        "2000": 130,
        "3000": 150,
        "4000": 160,
        "5000": 200,
        "10000": 240,
        "20000": null
      }
    },
    {
      "id": "craft20_3",
      "name": "凹凸",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft20_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft20_5",
      "name": "无色压凹（单面）",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft20_6",
      "name": "深凹烫金（单面）",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    }
  ],
  "paper2_22": [
    {
      "id": "craft22_1",
      "name": "烫金（单面）",
      "prices": {
        "1000": 90,
        "2000": 110,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 200,
        "20000": 280,
        "30000": 350,
        "50000": 450,
        "100000": 800
      }
    },
    {
      "id": "craft22_2",
      "name": "烫金（双面）",
      "prices": {
        "1000": 110,
        "2000": 130,
        "3000": 150,
        "4000": 160,
        "5000": 200,
        "10000": 240,
        "20000": 380,
        "30000": 500,
        "50000": 800,
        "100000": 1400
      }
    },
    {
      "id": "craft22_3",
      "name": "UV",
      "prices": {
        "1000": 100,
        "2000": 130,
        "3000": 140,
        "4000": 160,
        "5000": 180,
        "10000": 240,
        "20000": 300,
        "30000": 400,
        "50000": 600,
        "100000": null
      }
    },
    {
      "id": "craft22_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": 600,
        "30000": 800,
        "50000": 1300,
        "100000": 2500
      }
    },
    {
      "id": "craft22_5",
      "name": "凹凸",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": 280,
        "30000": 320,
        "50000": 400,
        "100000": 500
      }
    }
  ],
  "paper2_23": [
    {
      "id": "craft23_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 80,
        "5000": 90,
        "10000": 170,
        "20000": null
      }
    },
    {
      "id": "craft23_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 70,
        "2000": 80,
        "3000": 90,
        "4000": 100,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft23_3",
      "name": "凹凸",
      "prices": {
        "1000": 80,
        "2000": 90,
        "3000": 100,
        "4000": 110,
        "5000": 120,
        "10000": 230,
        "20000": null
      }
    },
    {
      "id": "craft23_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft23_5",
      "name": "丝印白（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 130,
        "20000": null
      }
    },
    {
      "id": "craft23_6",
      "name": "丝印白（双面）",
      "prices": {
        "1000": 50,
        "2000": 60,
        "3000": 70,
        "4000": 90,
        "5000": 90,
        "10000": 160,
        "20000": null
      }
    }
  ],
  "paper2_24": [
    {
      "id": "craft24_1",
      "name": "烫金 小面积（单面）",
      "prices": {
        "1000": 90,
        "2000": 110,
        "3000": 130,
        "4000": 140,
        "5000": 160,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft24_2",
      "name": "烫金 小面积（双面）",
      "prices": {
        "1000": 110,
        "2000": 130,
        "3000": 150,
        "4000": 160,
        "5000": 200,
        "10000": 240,
        "20000": null
      }
    },
    {
      "id": "craft24_3",
      "name": "凹凸",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft24_4",
      "name": "鸡眼",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft24_5",
      "name": "无色压凹（单面）",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    },
    {
      "id": "craft24_6",
      "name": "深凹烫金（单面）",
      "prices": {
        "1000": 110,
        "2000": 120,
        "3000": 130,
        "4000": 140,
        "5000": 150,
        "10000": 200,
        "20000": null
      }
    }
  ],
  "paper2_26": [
    {
      "id": "craft26_1",
      "name": "鸡眼（单面）",
      "prices": {
        "1000": 40,
        "2000": 60,
        "3000": 90,
        "4000": 120,
        "5000": 150,
        "10000": 300,
        "20000": null
      }
    },
    {
      "id": "craft26_2",
      "name": "鸡眼（对折两张）",
      "prices": {
        "1000": 60,
        "2000": 120,
        "3000": 180,
        "4000": 240,
        "5000": 300,
        "10000": 550,
        "20000": null
      }
    }
  ],
  "paper2_27": [
    {
      "id": "craft27_1",
      "name": "印白色（单面）",
      "prices": {
        "1000": 30,
        "2000": 40,
        "3000": 50,
        "4000": 60,
        "5000": 70,
        "10000": 120,
        "20000": null
      }
    }
  ]
};
const DEFAULT_ROPE_CONFIG = [
  { id: "rope0", name: "不加吊绳", prices: { 500: 0, 1000: 0, 2000: 0, 3000: 0, 5000: 0 } },
  { id: "rope1", name: "普通吊绳", prices: { 500: 10.0, 1000: 10.0, 2000: 20.0, 3000: 30.0, 5000: 50.0 } },
  { id: "rope2", name: "黑色方头", prices: { 500: 4.5, 1000: 4.5, 2000: 9.0, 3000: 13.5, 5000: 22.5 } },
  { id: "rope3", name: "白色方头", prices: { 500: 4.5, 1000: 4.5, 2000: 9.0, 3000: 13.5, 5000: 22.5 } },
  { id: "rope4", name: "加长加粗子弹头", prices: { 500: 20.0, 1000: 20.0, 2000: 40.0, 3000: 60.0, 5000: 100.0 } },
  { id: "rope5", name: "金色别针", prices: { 500: 12.0, 1000: 12.0, 2000: 24.0, 3000: 36.0, 5000: 60.0 } },
  { id: "rope6", name: "米色棉绳", prices: { 500: 9.0, 1000: 9.0, 2000: 18.0, 3000: 27.0, 5000: 45.0 } },
  { id: "rope7", name: "白色蜡绳", prices: { 500: 11.0, 1000: 11.0, 2000: 22.0, 3000: 33.0, 5000: 55.0 } },
  { id: "rope8", name: "黑色蜡绳", prices: { 500: 11.0, 1000: 11.0, 2000: 22.0, 3000: 33.0, 5000: 55.0 } },
  { id: "rope9", name: "黑色别针", prices: { 500: 12.0, 1000: 12.0, 2000: 24.0, 3000: 36.0, 5000: 60.0 } },
  { id: "rope10", name: "黑双插圆柱蜡绳", prices: { 500: 22.0, 1000: 22.0, 2000: 44.0, 3000: 66.0, 5000: 110.0 } },
  { id: "rope11", name: "白双插圆柱蜡绳", prices: { 500: 22.0, 1000: 22.0, 2000: 44.0, 3000: 66.0, 5000: 110.0 } },
  { id: "rope12", name: "白色双插丝带", prices: { 500: 22.0, 1000: 22.0, 2000: 44.0, 3000: 66.0, 5000: 110.0 } },
  { id: "rope13", name: "黑色双插丝带", prices: { 500: 22.0, 1000: 22.0, 2000: 44.0, 3000: 66.0, 5000: 110.0 } },
  { id: "rope14", name: "紫色双插涤纶", prices: { 500: 13.0, 1000: 13.0, 2000: 26.0, 3000: 39.0, 5000: 65.0 } },
  { id: "rope15", name: "粉色双插涤纶", prices: { 500: 13.0, 1000: 13.0, 2000: 26.0, 3000: 39.0, 5000: 65.0 } },
  { id: "rope16", name: "黑色双插涤纶", prices: { 500: 13.0, 1000: 13.0, 2000: 26.0, 3000: 39.0, 5000: 65.0 } },
  { id: "rope17", name: "白色双插涤纶", prices: { 500: 13.0, 1000: 13.0, 2000: 26.0, 3000: 39.0, 5000: 65.0 } },
  { id: "rope18", name: "白色单插蜡绳", prices: { 500: 20.0, 1000: 20.0, 2000: 40.0, 3000: 60.0, 5000: 100.0 } },
  { id: "rope19", name: "黑色单插蜡绳", prices: { 500: 20.0, 1000: 20.0, 2000: 40.0, 3000: 60.0, 5000: 100.0 } },
  { id: "rope20", name: "黑色双插蜡绳", prices: { 500: 22.0, 1000: 22.0, 2000: 44.0, 3000: 66.0, 5000: 110.0 } },
  { id: "rope21", name: "白色双插蜡绳", prices: { 500: 22.0, 1000: 22.0, 2000: 44.0, 3000: 66.0, 5000: 110.0 } },
  { id: "rope22", name: "防拆条", prices: { 500: 31.0, 1000: 31.0, 2000: 62.0, 3000: 93.0, 5000: 155.0 } },
  { id: "rope23", name: "空白防拆条", prices: { 500: 31.0, 1000: 31.0, 2000: 62.0, 3000: 93.0, 5000: 155.0 } },
  { id: "rope24", name: "棉带防拆条", prices: { 500: 51.0, 1000: 51.0, 2000: 102.0, 3000: 153.0, 5000: 255.0 } },
  { id: "rope25", name: "黑色光胶防拆条", prices: { 500: 47.0, 1000: 47.0, 2000: 94.0, 3000: 141.0, 5000: 235.0 } },
  { id: "rope26", name: "防拆扣", prices: { 500: 11.0, 1000: 11.0, 2000: 22.0, 3000: 33.0, 5000: 55.0 } },
];
// 吊绳配置为全局共享，不按报价表隔离

/**
 * 邮费规则配置：含小面积折扣阈值与折扣系数。
 * 邮费配置为全局共享，不按报价表隔离。
 */
let SHIPPING_CONFIG = [
  { id: "region1", name: "广东省内", basePrices: { 500: 8, 1000: 8, 2000: 13, 5000: 19, 10000: 26 }, overTierCoeff: 1 },
  { id: "region2", name: "福建、浙江、上海、江苏、湖南、安徽", basePrices: { 500: 20, 1000: 20, 2000: 27, 5000: 32, 10000: 42 }, overTierCoeff: 1.2 },
  { id: "region3", name: "湖北、江西、广西", basePrices: { 500: 20, 1000: 20, 2000: 27, 5000: 32, 10000: 42 }, overTierCoeff: 1.3 },
  { id: "region4", name: "河南、河北、天津、山东、山西、陕西", basePrices: { 500: 26, 1000: 26, 2000: 34, 5000: 42, 10000: 50 }, overTierCoeff: 1.7 },
  { id: "region5", name: "云南、贵州、重庆、四川", basePrices: { 500: 26, 1000: 26, 2000: 34, 5000: 42, 10000: 50 }, overTierCoeff: 1.7 },
  { id: "region6", name: "海南、北京", basePrices: { 500: 35, 1000: 35, 2000: 42, 5000: 52, 10000: 62 }, overTierCoeff: 2 },
  { id: "region7", name: "内蒙古、吉林、辽宁、甘肃、宁夏", basePrices: { 500: 40, 1000: 40, 2000: 47, 5000: 57, 10000: 67 }, overTierCoeff: 2.5 },
  { id: "region8", name: "黑龙江、青海", basePrices: { 500: 40, 1000: 40, 2000: 47, 5000: 57, 10000: 67 }, overTierCoeff: 3 },
  { id: "region9", name: "新疆、西藏", basePrices: { 500: 40, 1000: 40, 2000: 47, 5000: 57, 10000: 67 }, overTierCoeff: 4 }
];
const DEFAULT_SHIPPING_CONFIG = SHIPPING_CONFIG.map(s => ({ ...s, basePrices: { ...s.basePrices } }));

/**
 * 客户等级与毛利系数配置。
 */
const DEFAULT_CUSTOMER_LEVELS = [
  { id: "level1", name: "普通客户", coefficient: 1.2 },
  { id: "level2", name: "优质客户", coefficient: 1.15 },
  { id: "level3", name: "大客户", coefficient: 1.1 }
];

/**
 * 直接系数等级（仅用于"直接系数计算"模式的报价卡片展示）。
 * 直接系数模式下不计算吊绳和邮费，成本 = 纸张 + 工艺，报价 = 成本 × 直接系数。
 *
 * 注意：v6.2 起直接系数只读取报价表（Sheet）的「直接系数档位 / 最高倍数 / 最低倍数」，
 * 此处仅保留等级名称用于展示，系数值由表格按批量档位提供（最高倍数→普通客户，最低倍数→大客户，中间等级等差插值）。
 */
const DEFAULT_DIRECT_COEFF_LEVELS = [
  { id: "direct1", name: "普通客户", coefficient: 1.5 },
  { id: "direct2", name: "优质客户", coefficient: 1.45 },
  { id: "direct3", name: "大客户", coefficient: 1.4 }
];

/**
 * 持久化策略：纸张/工艺/吊绳/邮费/客户等级/报价表组配置均读写 localStorage。
 * 纸张按报价表隔离（通过 priceListId 字段），吊绳/邮费/客户等级为全局共享。
 * EPHEMERAL_KEYS 已提前定义为空数组，不拦截任何 key。
 */

function saveToStorage(key, value) {
  if (EPHEMERAL_KEYS.includes(key)) return; // 空数组，不拦截任何 key
  try {
    localStorage.setItem("tagPricing_" + key, JSON.stringify(value));
  } catch (e) {
    console.warn("localStorage 写入失败", e);
  }
}

function loadFromStorage(key, defaultValue) {
  if (EPHEMERAL_KEYS.includes(key)) return defaultValue; // 空数组，不拦截任何 key
  try {
    const raw = localStorage.getItem("tagPricing_" + key);
    return raw ? JSON.parse(raw) : defaultValue;
  } catch (e) {
    console.warn("localStorage 读取失败", e);
    return defaultValue;
  }
}

function generateId() {
  return Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
}


// 运行时配置：从 localStorage 加载，没有则用 DEFAULT
let PAPER_CONFIG = loadFromStorage("paperConfig", DEFAULT_PAPER_CONFIG.map(p => ({
  ...p,
  specs: p.specs.map(s => ({ ...s, prices: { ...s.prices } }))
})));
// v5.4 迁移：为旧版 PAPER_CONFIG 补充 priceListId 字段（默认归 "priceList1"）
(function migratePaperConfigPriceListId() {
  let changed = false;
  PAPER_CONFIG.forEach(p => {
    if (!p.priceListId) {
      p.priceListId = "priceList1";
      changed = true;
    }
  });
  if (changed) {
    saveToStorage("paperConfig", PAPER_CONFIG);
    console.info("[v5.4] 纸张配置已补充 priceListId 字段");
  }
})();
let ROPE_CONFIG = loadFromStorage("ropeConfig", DEFAULT_ROPE_CONFIG.map(r => ({ ...r, prices: { ...r.prices } })));
// v5.4 迁移：清理吊绳中遗留的 priceListId 字段（v5.3 临时实现遗留）
(function migrateRemoveRopePriceListId() {
  let changed = false;
  ROPE_CONFIG.forEach(r => {
    if ("priceListId" in r) { delete r.priceListId; changed = true; }
  });
  if (changed) {
    saveToStorage("ropeConfig", ROPE_CONFIG);
    console.info("[v5.4] 吊绳配置已清理 priceListId 字段");
  }
})();
let CRAFT_CONFIG = loadFromStorage("craftConfig", {});
if (!Object.keys(CRAFT_CONFIG).length) {
  Object.keys(DEFAULT_CRAFT_CONFIG).forEach(k => {
    CRAFT_CONFIG[k] = DEFAULT_CRAFT_CONFIG[k].map(c => ({ ...c, prices: { ...c.prices } }));
  });
}
SHIPPING_CONFIG = loadFromStorage("shippingConfig", DEFAULT_SHIPPING_CONFIG.map(s => ({ ...s, basePrices: { ...s.basePrices } })));
// v5.4 迁移：清理邮费中遗留的 priceListId 字段（v5.3 临时实现遗留）
(function migrateRemoveShippingPriceListId() {
  let changed = false;
  SHIPPING_CONFIG.forEach(s => {
    if ("priceListId" in s) { delete s.priceListId; changed = true; }
  });
  if (changed) {
    saveToStorage("shippingConfig", SHIPPING_CONFIG);
    console.info("[v5.4] 邮费配置已清理 priceListId 字段");
  }
})();
// 客户等级（毛利系数）持久化到 localStorage，刷新后保留用户自定义
let CUSTOMER_LEVELS = loadFromStorage("customerLevels", DEFAULT_CUSTOMER_LEVELS.map(l => ({ ...l })));

// 直接系数等级（固定展示名称，v6.2 起直接系数只读取报价表表格字段）
// 系数值由表格按批量档位提供：最高倍数→普通客户，最低倍数→大客户，中间等级等差插值。
// 不再支持个人主页编辑，也不再持久化到 localStorage。
const DIRECT_COEFF_LEVELS = DEFAULT_DIRECT_COEFF_LEVELS.map(l => ({ ...l }));

// v5.2 迁移：旧版默认系数（1.3 / 1.15 / 1.05）自动更新为新版（1.2 / 1.15 / 1.1）
// 仅当 localStorage 中的值与旧默认完全一致时才迁移，用户自定义值不受影响
(function migrateCustomerLevels() {
  const OLD_DEFAULTS = [
    { name: "普通客户", coefficient: 1.3 },
    { name: "优质客户", coefficient: 1.15 },
    { name: "大客户", coefficient: 1.05 }
  ];
  const isOldDefault = CUSTOMER_LEVELS.length === 3 &&
    CUSTOMER_LEVELS.every((lv, i) => lv.name === OLD_DEFAULTS[i].name && lv.coefficient === OLD_DEFAULTS[i].coefficient);
  if (isOldDefault) {
    CUSTOMER_LEVELS = DEFAULT_CUSTOMER_LEVELS.map(l => ({ ...l }));
    saveToStorage("customerLevels", CUSTOMER_LEVELS);
    console.info("[v5.2] 客户等级系数已从旧默认迁移至新默认");
  }
})();

// v6.1 迁移：为旧版 PAPER_CONFIG 补充 602米白卡（paper10）
(function migrateAddPaper10() {
  const hasPaper10 = PAPER_CONFIG.some(p => p.id === "paper10");
  if (!hasPaper10) {
    const p10 = DEFAULT_PAPER_CONFIG.find(p => p.id === "paper10");
    if (p10) {
      PAPER_CONFIG.push(JSON.parse(JSON.stringify(p10)));
      saveToStorage("paperConfig", PAPER_CONFIG);
      console.info("[v6.1] 已补充 602米白卡 纸张");
    }
  }
})();
// v6.1 迁移：为旧版 CRAFT_CONFIG 补充 602米白卡 工艺
(function migrateAddPaper10Craft() {
  if (!CRAFT_CONFIG["paper10"] && DEFAULT_CRAFT_CONFIG["paper10"]) {
    CRAFT_CONFIG["paper10"] = DEFAULT_CRAFT_CONFIG["paper10"].map(c => ({ ...c, prices: { ...c.prices } }));
    saveToStorage("craftConfig", CRAFT_CONFIG);
    console.info("[v6.1] 已补充 602米白卡 工艺配置");
  }
})();
// v6.2 迁移：更新所有纸张的直接系数配置为表格格式（tiers/max/min 数组，无系数为 null）
(function migrateDirectCoeffToTableFormat() {
  let changed = false;
  PAPER_CONFIG.forEach(p => {
    const def = DEFAULT_PAPER_CONFIG.find(d => d.id === p.id);
    const defDC = def ? def.directCoeff : null;
    // 旧格式：{ tierRules: [...] } 或 { tierRules: [] } → 更新为新格式
    if (p.directCoeff && p.directCoeff.tierRules !== undefined) {
      // 如果有 tierRules，从默认配置中获取新格式
      p.directCoeff = defDC ? JSON.parse(JSON.stringify(defDC)) : null;
      changed = true;
    } else if (p.directCoeff === undefined || p.directCoeff === null) {
      // 没有 directCoeff → 使用默认配置
      p.directCoeff = defDC ? JSON.parse(JSON.stringify(defDC)) : null;
      changed = true;
    }
  });
  if (changed) {
    saveToStorage("paperConfig", PAPER_CONFIG);
    console.info("[v6.2] 已更新所有纸张直接系数为表格格式");
  }
})();

// v7.5 迁移：为旧版存储补充 2/3/4 号报价表及其纸张、工艺默认数据
(function migrateAddDefaultPriceLists() {
  let changed = false;
  // 1) 合并默认报价表组（2/3/4 号）
  DEFAULT_PRICE_LISTS.forEach(def => {
    if (!PRICE_LISTS.some(p => p.id === def.id)) {
      PRICE_LISTS.push({ ...def });
      changed = true;
    }
  });
  // 2) 补充默认纸张（按 priceListId 分组，缺失则加入）
  DEFAULT_PAPER_CONFIG.forEach(def => {
    if (!PAPER_CONFIG.some(p => p.id === def.id)) {
      PAPER_CONFIG.push(JSON.parse(JSON.stringify(def)));
      changed = true;
    }
  });
  // 3) 补充默认工艺（按纸张 id 分组，缺失则加入）
  Object.keys(DEFAULT_CRAFT_CONFIG).forEach(k => {
    if (!CRAFT_CONFIG[k] && DEFAULT_CRAFT_CONFIG[k]) {
      CRAFT_CONFIG[k] = DEFAULT_CRAFT_CONFIG[k].map(c => ({ ...c, prices: { ...c.prices } }));
      changed = true;
    }
  });
  if (changed) {
    saveToStorage("priceLists", PRICE_LISTS);
    saveToStorage("paperConfig", PAPER_CONFIG);
    saveToStorage("craftConfig", CRAFT_CONFIG);
    console.info("[v7.5] 已补充 2/3/4 号报价表默认数据");
  }
})();

// v7.8 迁移：为旧版存储补充 702铜版纸（2号报价表-3，id=paper2_16）并调整到正确顺序
// 说明：v7.5 迁移虽会按 DEFAULT 补充缺失纸张，但 push 到数组末尾，导致 702铜版纸 排在 2号报价表最后而非第 3 位，这里做重排。
(function migrateAddPaper2_16() {
  let changed = false;
  // 1) 补充 702铜版纸 纸张（缺失则从默认配置深拷贝）
  const defPaper = DEFAULT_PAPER_CONFIG.find(p => p.id === "paper2_16");
  if (defPaper && !PAPER_CONFIG.some(p => p.id === "paper2_16")) {
    PAPER_CONFIG.push(JSON.parse(JSON.stringify(defPaper)));
    changed = true;
  }
  // 2) 补充 702铜版纸 工艺（缺失则从默认配置深拷贝）
  if (DEFAULT_CRAFT_CONFIG["paper2_16"] && !CRAFT_CONFIG["paper2_16"]) {
    CRAFT_CONFIG["paper2_16"] = DEFAULT_CRAFT_CONFIG["paper2_16"].map(c => ({ ...c, prices: { ...c.prices } }));
    changed = true;
  }
  // 3) 将 paper2_16 移动到 paper2_2 之后（2号报价表第 3 位）
  const idx16 = PAPER_CONFIG.findIndex(p => p.id === "paper2_16");
  const idx2 = PAPER_CONFIG.findIndex(p => p.id === "paper2_2");
  if (idx16 !== -1 && idx2 !== -1) {
    const [item] = PAPER_CONFIG.splice(idx16, 1);
    const newIdx = PAPER_CONFIG.findIndex(p => p.id === "paper2_2");
    PAPER_CONFIG.splice(newIdx + 1, 0, item);
    changed = true;
  }
  if (changed) {
    saveToStorage("paperConfig", PAPER_CONFIG);
    saveToStorage("craftConfig", CRAFT_CONFIG);
    console.info("[v7.8] 已补充 702铜版纸 默认数据并调整顺序");
  }
})();

// v8.0 迁移：重构为 2 个报价表（1楼 34 纸 + 3楼 10 纸），覆盖纸张与邮费数据
(function migrateV8Restructure() {
  // 旧结构特征：报价表数 > 2，或存在 3/4 号报价表，或报价表名不含"楼"
  const isOld = PRICE_LISTS.length > 2
    || PRICE_LISTS.some(p => p.id === "priceList3" || p.id === "priceList4")
    || PRICE_LISTS.some(p => !/楼/.test(p.name || ""));
  if (!isOld) return;

  PRICE_LISTS = DEFAULT_PRICE_LISTS.map(p => ({ ...p }));
  saveToStorage("priceLists", PRICE_LISTS);

  PAPER_CONFIG = DEFAULT_PAPER_CONFIG.map(p => JSON.parse(JSON.stringify(p)));
  saveToStorage("paperConfig", PAPER_CONFIG);

  SHIPPING_CONFIG = DEFAULT_SHIPPING_CONFIG.map(s => ({ ...s, basePrices: { ...s.basePrices } }));
  saveToStorage("shippingConfig", SHIPPING_CONFIG);

  CURRENT_PRICE_LIST_ID = "priceList1";
  saveToStorage("currentPriceListId", CURRENT_PRICE_LIST_ID);

  console.info("[v8.0] 已重构为 2 个报价表（1楼/3楼）并覆盖纸张与邮费数据");
})();

// 版本号比较（按点分段数字比较，如 "9.5.1" >= "9.0"）
function verCompare(a, b) {
  const pa = String(a || "0").split(".").map(Number);
  const pb = String(b || "0").split(".").map(Number);
  for (let i = 0; i < Math.max(pa.length, pb.length); i++) {
    const x = pa[i] || 0, y = pb[i] || 0;
    if (x > y) return 1; if (x < y) return -1;
  }
  return 0;
}

// v8.4 迁移：用纠正版 Excel（KOKALabel1楼.xlsx / KOKALabel3楼.xlsx）覆盖纸张与工艺为最新默认数据
(function migrateV8_4CorrectData() {
  const dataVersion = loadFromStorage("dataVersion", "");
  if (verCompare(dataVersion, "8.4") >= 0) return;

  PAPER_CONFIG = DEFAULT_PAPER_CONFIG.map(p => JSON.parse(JSON.stringify(p)));
  saveToStorage("paperConfig", PAPER_CONFIG);

  Object.keys(DEFAULT_CRAFT_CONFIG).forEach(k => {
    CRAFT_CONFIG[k] = DEFAULT_CRAFT_CONFIG[k].map(c => ({ ...c, prices: { ...c.prices } }));
  });
  saveToStorage("craftConfig", CRAFT_CONFIG);

  saveToStorage("dataVersion", "8.4");
  console.info("[v8.4] 已按最新 Excel 覆盖纸张与工艺数据");
})();

// v9.0 迁移：用最新 Excel 替换 1楼报价表（34 张 → 报价组），保留 3楼（含用户自定义）
// v9.0.1：最新附件为 28 个报价组（KOKALabel1楼_20260904_1531(1).xlsx）
(function migrateV9ReplaceFloor1() {
  const dataVersion = loadFromStorage("dataVersion", "");
  if (verCompare(dataVersion, "9.0") >= 0) return;

  // 3楼 纸 id（paper1~paper10）
  const floor3Ids = new Set(DEFAULT_PAPER_CONFIG.filter(p => p.priceListId === "priceList2").map(p => p.id));

  // 纸张：保留 localStorage 中的 3楼（含用户改价），1楼 用新默认报价组
  const storedPapers = loadFromStorage("paperConfig", null);
  if (Array.isArray(storedPapers) && storedPapers.length) {
    const keptFloor3 = storedPapers.filter(p => floor3Ids.has(p.id));
    const newFloor1 = DEFAULT_PAPER_CONFIG.filter(p => !floor3Ids.has(p.id)).map(p => JSON.parse(JSON.stringify(p)));
    PAPER_CONFIG = keptFloor3.concat(newFloor1);
  } else {
    PAPER_CONFIG = DEFAULT_PAPER_CONFIG.map(p => JSON.parse(JSON.stringify(p)));
  }
  saveToStorage("paperConfig", PAPER_CONFIG);

  // 工艺：保留 3楼 key，1楼（新 paper2_x）按新默认重建
  const storedCrafts = loadFromStorage("craftConfig", null);
  if (storedCrafts && typeof storedCrafts === "object") {
    const kept = {};
    Object.keys(storedCrafts).forEach(k => { if (floor3Ids.has(k)) kept[k] = storedCrafts[k]; });
    Object.keys(DEFAULT_CRAFT_CONFIG).forEach(k => {
      if (!floor3Ids.has(k)) kept[k] = DEFAULT_CRAFT_CONFIG[k].map(c => ({ ...c, prices: { ...c.prices } }));
    });
    CRAFT_CONFIG = kept;
  } else {
    CRAFT_CONFIG = JSON.parse(JSON.stringify(DEFAULT_CRAFT_CONFIG));
  }
  saveToStorage("craftConfig", CRAFT_CONFIG);

  saveToStorage("dataVersion", "9.0.1");
  console.info("[v9.0.1] 已用最新 Excel 替换 1楼报价表（28 组），3楼保留");
})();

// 注：原 v9.3.1「900克蒙版纸改名」迁移已移除——其守卫被历次版本号批量替换污染（9.3.1→9.4.0→9.5.0→9.5.1），
// 且该改名已被 v9.5.1 的 1楼整体更新覆盖，保留反而会抢先消耗 dataVersion 导致 1楼更新迁移被跳过。
// v9.5.1 迁移：1楼报价表更新为 09-09 附件（33 组，新增 800纹棉/400木香组合/180闪银/200云彩/800木香）
// 规则与 v9.0.1 一致：保留 3楼（含用户改价），1楼纸张与工艺整体替换为新默认
(function migrateFloor1V951() {
  const dataVersion = loadFromStorage("dataVersion", "");
  if (verCompare(dataVersion, "9.5.1") >= 0) return;
  const stored = loadFromStorage("paperConfig", null);
  if (Array.isArray(stored) && stored.length) {
    const floor3 = stored.filter(p => p.priceListId === "priceList2");
    const floor1 = DEFAULT_PAPER_CONFIG.filter(p => p.priceListId === "priceList1").map(p => JSON.parse(JSON.stringify(p)));
    PAPER_CONFIG = floor1.concat(floor3); // v10.6.0 修复：同事版只写存储不同步内存，导致本会话残留旧数据，需刷新才正常
    saveToStorage("paperConfig", PAPER_CONFIG);
  }
  const storedCraft = loadFromStorage("craftConfig", null);
  if (storedCraft && typeof storedCraft === "object") {
    Object.keys(storedCraft).forEach(k => { if (/^paper2_\d+$/.test(k)) delete storedCraft[k]; });
    Object.keys(DEFAULT_CRAFT_CONFIG).forEach(k => {
      if (/^paper2_\d+$/.test(k)) storedCraft[k] = DEFAULT_CRAFT_CONFIG[k].map(c => ({ ...c, prices: { ...c.prices } }));
    });
    CRAFT_CONFIG = storedCraft; // v10.6.0 修复：同 PAPER_CONFIG，同步内存
    saveToStorage("craftConfig", CRAFT_CONFIG);
  }
  saveToStorage("dataVersion", "9.5.1");
  console.info("[v9.5.1] 1楼报价表已更新（33 组，09-09 附件）");
})();

// v9.9.0 迁移：吊绳价格切换为 1000 张基准倍数制——500 张档位价格改为与 1000 张同价
// （旧数据 500 档为半价；计算逻辑 v9.9 起按 基准×档位/1000 计算，不再查精确档位）
(function migrateRopeV99() {
  const dataVersion = loadFromStorage("dataVersion", "");
  if (verCompare(dataVersion, "9.9.0") >= 0) return;
  const stored = loadFromStorage("ropeConfig", null);
  if (Array.isArray(stored) && stored.length) {
    let changed = false;
    stored.forEach(r => {
      const prices = r && r.prices;
      if (prices && prices["1000"] != null && prices["1000"] !== "" && prices["500"] !== prices["1000"]) {
        prices["500"] = prices["1000"];
        changed = true;
      }
    });
    if (changed) {
      ROPE_CONFIG = stored;
      saveToStorage("ropeConfig", ROPE_CONFIG);
    }
  }
  saveToStorage("dataVersion", "9.9.0");
  console.info("[v9.9.0] 吊绳价格已切换为 1000 张基准倍数制（500 张与 1000 张同价）");
})();

let APP_PROFILE = loadFromStorage("appProfile", {
  companyName: "KOKALabel",
  companyPhone: null,
  defaultTier: null,
  defaultRope: "rope1",
  defaultPaperId: null,
  defaultSizeType: "single",
  hidePaperIndex: true,
  paperSelectMode: "grid",
  decimalPlaces: 2
});
// 旧版配置容错
if (typeof APP_PROFILE.defaultRope === "undefined") APP_PROFILE.defaultRope = "rope1";
if (typeof APP_PROFILE.defaultPaperId === "undefined") APP_PROFILE.defaultPaperId = "";
if (typeof APP_PROFILE.defaultSizeType === "undefined") APP_PROFILE.defaultSizeType = "single";
// v10.0：纸张材质序号显示开关（默认隐藏）
if (typeof APP_PROFILE.hidePaperIndex === "undefined") APP_PROFILE.hidePaperIndex = true;
// v10.1：纸张材质选择模式（grid=芯片平铺默认 / dropdown=下拉搜索）
if (typeof APP_PROFILE.paperSelectMode === "undefined") APP_PROFILE.paperSelectMode = "grid";

// ============================================================
// ===================== 报价表组辅助函数 =====================
// ============================================================

/** 返回指定报价表的纸张子集 */
function getPapersByPriceList(priceListId) {
  return PAPER_CONFIG.filter(p => p.priceListId === priceListId);
}
/** 返回当前报价表 ID */
function getCurrentPriceListId() {
  return CURRENT_PRICE_LIST_ID;
}
/** 获取当前报价表对象 */
function getCurrentPriceList() {
  return PRICE_LISTS.find(p => p.id === CURRENT_PRICE_LIST_ID) || PRICE_LISTS[0];
}
/** 切换当前报价表 */
function setCurrentPriceList(priceListId) {
  if (PRICE_LISTS.find(p => p.id === priceListId)) {
    CURRENT_PRICE_LIST_ID = priceListId;
    saveToStorage("currentPriceListId", CURRENT_PRICE_LIST_ID);
  }
}
/**
 * 新增报价表并切换为当前报价表。
 * @param {string} name 报价表名称
 * @param {string} [groupId] 所属报价表组 ID（v10.7.0：缺省沿用当前小组）
 * @param {boolean} [switchTo=true] 是否切换为当前报价表（v10.7.0：在线管理区新增时不抢焦点）
 * @returns {string} 新报价表 ID
 */
function addPriceList(name, groupId, switchTo) {
  // v10.7.0：追加随机后缀防碰撞（同一毫秒内连续新增会生成相同纯时间戳 ID，对齐 v9.8 修复）
  const id = "priceList_" + Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
  PRICE_LISTS.push({ id, name, groupId: groupId || GROUP_META.id });
  saveToStorage("priceLists", PRICE_LISTS);
  if (switchTo !== false) {
    CURRENT_PRICE_LIST_ID = id;
    saveToStorage("currentPriceListId", CURRENT_PRICE_LIST_ID);
  }
  return id;
}
/**
 * 删除指定报价表及其关联纸张和工艺。
 * 不允许删除最后一个报价表。
 * @param {string} priceListId 报价表 ID
 * @returns {boolean} 是否删除成功
 */
function deletePriceList(priceListId) {
  if (PRICE_LISTS.length <= 1) return false;
  const idx = PRICE_LISTS.findIndex(p => p.id === priceListId);
  if (idx === -1) return false;
  // 删除该报价表的所有纸张
  PAPER_CONFIG = PAPER_CONFIG.filter(p => p.priceListId !== priceListId);
  saveToStorage("paperConfig", PAPER_CONFIG);
  // 清理无主工艺
  const validPaperIds = new Set(PAPER_CONFIG.map(p => p.id));
  Object.keys(CRAFT_CONFIG).forEach(k => {
    if (!validPaperIds.has(k)) delete CRAFT_CONFIG[k];
  });
  saveToStorage("craftConfig", CRAFT_CONFIG);
  // 删除报价表
  PRICE_LISTS.splice(idx, 1);
  saveToStorage("priceLists", PRICE_LISTS);
  // 如果删除的是当前报价表，切换到第一个
  if (CURRENT_PRICE_LIST_ID === priceListId) {
    CURRENT_PRICE_LIST_ID = PRICE_LISTS[0].id;
    saveToStorage("currentPriceListId", CURRENT_PRICE_LIST_ID);
  }
  return true;
}

// ============================================================
// ============ v10.7.0 在线修改：报价表组/报价表管理 ============
// ============ （对齐 v9.8.0 体系，加回 F1/F2/F3） ============
// ============================================================

/** 返回全部报价表组 */
function getPriceListGroups() {
  return PRICE_LIST_GROUPS;
}

/** 按 ID 取组名（兼容旧数据：组不存在时回退 GROUP_NAME_MAP） */
function getGroupName(groupId) {
  const g = PRICE_LIST_GROUPS.find(x => x.id === groupId);
  if (g) return g.name;
  return GROUP_NAME_MAP[groupId] || "未分组";
}

/**
 * 新增报价表组。
 * @param {string} name 组名（去首尾空格，非空）
 * @returns {{ok: boolean, id?: string, message?: string}}
 */
function addPriceListGroup(name) {
  const trimmed = String(name == null ? "" : name).trim();
  if (!trimmed) return { ok: false, message: "组名不能为空" };
  if (PRICE_LIST_GROUPS.some(g => g.name === trimmed)) return { ok: false, message: "已存在同名报价表组" };
  // 随机后缀防碰撞（同 addPriceList）
  const id = "group_" + Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
  PRICE_LIST_GROUPS.push({ id, name: trimmed });
  saveToStorage("priceListGroups", PRICE_LIST_GROUPS);
  return { ok: true, id };
}

/**
 * 重命名报价表组（同步迁移 GROUP_NAME_MAP，保证导出 Excel 元信息一致）。
 * @param {string} groupId 组 ID
 * @param {string} newName 新组名
 * @returns {{ok: boolean, message?: string}}
 */
function renamePriceListGroup(groupId, newName) {
  const trimmed = String(newName == null ? "" : newName).trim();
  const group = PRICE_LIST_GROUPS.find(g => g.id === groupId);
  if (!group) return { ok: false, message: "报价表组不存在" };
  if (!trimmed) return { ok: false, message: "组名不能为空" };
  if (PRICE_LIST_GROUPS.some(g => g.id !== groupId && g.name === trimmed)) {
    return { ok: false, message: "已存在同名报价表组" };
  }
  group.name = trimmed;
  // 同步常量映射（导出 Excel「所属小组」行仍读该映射）
  if (groupId in GROUP_NAME_MAP) GROUP_NAME_MAP[groupId] = trimmed;
  saveToStorage("priceListGroups", PRICE_LIST_GROUPS);
  return { ok: true };
}

/**
 * 删除报价表组。仅允许删除空组且非最后一个组。
 * @param {string} groupId 组 ID
 * @returns {{ok: boolean, message?: string}}
 */
function deletePriceListGroup(groupId) {
  const idx = PRICE_LIST_GROUPS.findIndex(g => g.id === groupId);
  if (idx === -1) return { ok: false, message: "报价表组不存在" };
  if (PRICE_LIST_GROUPS.length <= 1) return { ok: false, message: "至少保留一个报价表组" };
  if (PRICE_LISTS.some(p => p.groupId === groupId)) return { ok: false, message: "组内仍有报价表，请先删除或移动" };
  PRICE_LIST_GROUPS.splice(idx, 1);
  saveToStorage("priceListGroups", PRICE_LIST_GROUPS);
  return { ok: true };
}

/**
 * 重命名报价表。
 * @param {string} priceListId 报价表 ID
 * @param {string} newName 新名称
 * @returns {{ok: boolean, message?: string}}
 */
function renamePriceList(priceListId, newName) {
  const pl = PRICE_LISTS.find(p => p.id === priceListId);
  if (!pl) return { ok: false, message: "报价表不存在" };
  const trimmed = String(newName == null ? "" : newName).trim();
  if (!trimmed) return { ok: false, message: "报价表名称不能为空" };
  if (PRICE_LISTS.some(p => p.id !== priceListId && p.name === trimmed)) {
    return { ok: false, message: "已存在同名报价表" };
  }
  pl.name = trimmed;
  saveToStorage("priceLists", PRICE_LISTS);
  return { ok: true };
}

/**
 * 把报价表移动到指定组。
 * @param {string} priceListId 报价表 ID
 * @param {string} groupId 目标组 ID
 * @returns {{ok: boolean, message?: string}}
 */
function movePriceListToGroup(priceListId, groupId) {
  const pl = PRICE_LISTS.find(p => p.id === priceListId);
  if (!pl) return { ok: false, message: "报价表不存在" };
  if (!PRICE_LIST_GROUPS.some(g => g.id === groupId)) return { ok: false, message: "目标报价表组不存在" };
  pl.groupId = groupId;
  saveToStorage("priceLists", PRICE_LISTS);
  return { ok: true };
}

/**
 * 在线修改：把解析后的 Excel 数据原地覆盖到指定报价表（不新建报价表）。
 * 旧纸张与旧工艺会先清理，再合入新数据。
 * @param {string} priceListId 目标报价表 ID
 * @param {{papers: Array, crafts: Object}} parsed parsePaperExcel 的解析结果
 * @returns {{ok: boolean, message?: string, paperCount?: number, craftCount?: number}}
 */
function applyPriceListData(priceListId, parsed) {
  const pl = PRICE_LISTS.find(p => p.id === priceListId);
  if (!pl) return { ok: false, message: "目标报价表不存在" };
  const papers = (parsed && parsed.papers) || [];
  if (!papers.length) return { ok: false, message: "解析结果中没有纸张数据" };
  // 1. 清理目标报价表的旧纸张及其工艺
  const removedPaperIds = new Set(PAPER_CONFIG.filter(p => p.priceListId === priceListId).map(p => p.id));
  PAPER_CONFIG = PAPER_CONFIG.filter(p => p.priceListId !== priceListId);
  const crafts = parsed.crafts || {};
  Object.keys(CRAFT_CONFIG).forEach(pid => {
    if (removedPaperIds.has(pid) && !crafts[pid]) delete CRAFT_CONFIG[pid];
  });
  // 2. 写入新纸张（绑定目标报价表）并合入工艺
  papers.forEach(p => { p.priceListId = priceListId; });
  PAPER_CONFIG = PAPER_CONFIG.concat(papers);
  CRAFT_CONFIG = { ...CRAFT_CONFIG, ...crafts };
  saveToStorage("paperConfig", PAPER_CONFIG);
  saveToStorage("craftConfig", CRAFT_CONFIG);
  const craftCount = Object.values(crafts).reduce((sum, arr) => sum + (arr ? arr.length : 0), 0);
  return { ok: true, paperCount: papers.length, craftCount };
}


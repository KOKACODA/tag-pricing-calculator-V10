...
test("v9.7.3 迁移：老用户（9.0.1）1 楼替换为 33 组，3 楼改价保留，工艺重建", () => {
  const store = legacyStore();
  const api = loadDataJs(store);

  assert.equal(api.dataVersion, "9.7.3");
...
test("v9.7.3 迁移幂等：已是 9.7.3 的用户再次加载不重复迁移", () => {
  const store = legacyStore();
  loadDataJs(store); // 第一次：迁移至 9.7.3
  assert.equal(JSON.parse(store.get("tagPricing_dataVersion")), "9.7.3"); // 存储值经过 JSON.stringify
  ...
  assert.equal(api2.dataVersion, "9.7.3");
});
